...
// ==========================================
// 13. EKSİK OLAN TÜM FONKSİYONLAR (HATA ÇÖZÜMÜ)
// ==========================================

// 1. Aşağı Çekerek Yenileme (Pull to Refresh) Fonksiyonu
function f5At() {
    oyunSesi('dokun');
    piyasayiYenile();
    ozelUyari("Piyasa başarıyla güncellendi.", "bilgi");
}

// 2. İstatistik Sayfası
function istatistikleriGuncelle() {
    let elSatilan = document.getElementById('ist-satilan');
    let elGelir = document.getElementById('ist-gelir');
    let elGider = document.getElementById('ist-gider');
    let elNet = document.getElementById('ist-net');
    
    if(elSatilan) elSatilan.innerText = toplamSatilanArac;
    if(elGelir) elGelir.innerText = paraFormatla(toplamGelir);
    if(elGider) elGider.innerText = paraFormatla(toplamGider);
    
    let net = toplamGelir - toplamGider;
    if(elNet) {
        elNet.innerText = paraFormatla(net);
        elNet.style.color = net < 0 ? '#e74c3c' : (net > 0 ? '#27ae60' : '#2d3436');
    }
}

// 3. Personel Yönetimi ve Prim
function personelEkraniGuncelle() {
    let roller = [
        {id: 'usta', isim: 'Usta Başı', ucret: 25000},
        {id: 'smUzman', isim: 'Sosyal Medya Uzmanı', ucret: 15000},
        {id: 'satisTemsilci', isim: 'Satış Temsilcisi', ucret: 30000},
        {id: 'vale', isim: 'Vale / Yıkamacı', ucret: 10000},
        {id: 'eksper', isim: 'Özel Eksper', ucret: 20000},
        {id: 'avukat', isim: 'Şirket Avukatı', ucret: 40000}
    ];
    
    roller.forEach(r => {
        let div = document.getElementById('btn-personel-' + r.id);
        if(div) {
            if(personeller[r.id]) {
                div.innerHTML = `<button class="btn btn-kirmizi" onclick="personelIslem('${r.id}', ${r.ucret}, false)">İşten Çıkar</button>`;
            } else {
                div.innerHTML = `<button class="btn btn-yesil" onclick="personelIslem('${r.id}', ${r.ucret}, true)">İşe Al (${paraFormatla(r.ucret)} ₺/Ay)</button>`;
            }
        }
    });
    
    let mBar = document.getElementById('moral-bar');
    let mText = document.getElementById('moral-text');
    if(mBar) mBar.style.width = personelMorali + '%';
    
    if(mText) {
        if(personelMorali > 80) mText.innerText = `Durum: Mükemmel (%${personelMorali})`;
        else if(personelMorali > 50) mText.innerText = `Durum: İdare Eder (%${personelMorali})`;
        else if(personelMorali > 20) mText.innerText = `Durum: Kötü (%${personelMorali})`;
        else mText.innerText = `Durum: İSYAN ÇIKABİLİR (%${personelMorali})`;
    }
    
    if(mBar) {
        if(personelMorali > 80) mBar.style.background = '#00b894';
        else if(personelMorali > 40) mBar.style.background = '#f1c40f';
        else mBar.style.background = '#e74c3c';
    }
}

function personelIslem(id, ucret, iseAl) {
    if(iseAl) {
        if(!bakiyeYeterliMi(ucret)) { ozelUyari("İlk maaşı ödeyecek limitin yok!", "hata"); return; }
        personeller[id] = true;
        paramiz -= ucret;
        toplamGider += ucret;
        oyunSesi('kasa');
        ozelUyari("Personel işe alındı. İlk maaşı peşin ödendi.", "basari");
    } else {
        personeller[id] = false;
        oyunSesi('dokun');
        ozelUyari("Personel işten çıkarıldı.", "bilgi");
    }
    aktifEkraniYenile();
    oyunuKaydet();
}
...
function primDagit() {
    if(!bakiyeYeterliMi(15000)) { ozelUyari("Prim dağıtacak paran yok!", "hata"); return; }
    if(personelMorali >= 100) { ozelUyari("Personel zaten yeterince motive!", "bilgi"); return; }
    ...
    aktifEkraniYenile();
    oyunuKaydet();
    ozelUyari("Personele 15.000 ₺ prim dağıtıldı! Moral yükseldi.", "basari");
}

// 4. Dükkan Büyütme İşlemleri
function dukkanEkraniniGuncelle() {
    let s = seviyeler[dukkanSeviyesi - 1];
    let dIsim = document.getElementById('dukkan-isim');
    let dKap = document.getElementById('dukkan-kapasite');
    let yukAlan = document.getElementById('yukseltme-alani');
    
    if(dIsim) dIsim.innerText = "Seviye " + s.seviye + ": " + s.isim;
    if(dKap) dKap.innerText = s.kapasite === 999 ? "Sınırsız" : s.kapasite + " Araç";
    
    if(yukAlan) {
        if(dukkanSeviyesi < seviyeler.length) {
            let ns = seviyeler[dukkanSeviyesi];
            yukAlan.style.display = 'block';
            document.getElementById('yeni-seviye-isim').innerText = ns.isim + " (Kapasite: " + (ns.kapasite === 999 ? "Sınırsız" : ns.kapasite) + ")";
            document.getElementById('yeni-seviye-fiyat').innerText = paraFormatla(ns.fiyat) + " ₺";
        } else {
            yukAlan.innerHTML = `<div style="text-align:center; color:#27ae60; font-weight:bold; font-size:18px; margin-top:20px;"><i class='bx bxs-check-circle'></i> Holdingin zirvesindesin! Dükkan son seviye.</div>`;
        }
    }
}
...
function dukkaniYukselt() {
    ...
    if(!bakiyeYeterliMi(ns.fiyat)) { ozelUyari("Dükkanı büyütmek için bütçeniz yetersiz!", "hata"); return; }
    
    paramiz -= ns.fiyat;
    toplamGider += ns.fiyat;
    dukkanSeviyesi++;
    aracKapasitesi = ns.kapasite;
    
    oyunSesi('kasa');
    aktifEkraniYenile();
    oyunuKaydet();
    ozelUyari("Tebrikler! Dükkan seviye atladı ve kapasite arttı.", "basari");
}

// 5. Rent A Car (Araç Kiralama) Motoru
function kirayaVer(id) {
    const arabaIndex = garaj.findIndex(x => x.id === id);
    if(arabaIndex === -1) return;
    let araba = garaj[arabaIndex];
    
    if(araba.tamirDurumu > 0 || !araba.muayeneVar || araba.gumrukKalanGun > 0) { 
        ozelUyari("Hasarlı, muayenesiz veya gümrüklü araçlar kiralanamaz!", "hata"); 
        return; 
    }
    
    araba.teklifler = []; // Araç kiraya çıkınca gelen teklifleri sil
    ...
    aktifEkraniYenile();
    oyunuKaydet();
    ozelUyari(`${araba.marka} ${araba.model} aracı filoya eklendi! Artık günlük para basacak.`, "basari");
}

function kiradanCek(id) {
    const arabaIndex = rentACarFilosu.findIndex(x => x.id === id);
    if(arabaIndex === -1) return;
    ...
    aktifEkraniYenile();
    oyunuKaydet();
    ozelUyari(`${araba.marka} ${araba.model} garaja geri çekildi. Artık satabilirsin.`, "bilgi");
}

function rentACarEkraniGuncelle() {
    const lst = document.getElementById('rentacar-listesi');
    const blg = document.getElementById('rentacar-bilgi');
    if(!lst) return;
    lst.innerHTML = '';
    ...
    if(rentACarFilosu.length === 0) {
        ...
        rentACarFilosu.forEach(a => {
            let gunlukKira = Math.floor(a.fiyat * 0.005); // Değerinin binde beşi günlük kira
            lst.innerHTML += `
            <div class="ilan-karti" style="border-left: 5px solid #f39c12; flex-direction: column; align-items: flex-start;">
                <div style="display:flex; justify-content:space-between; width:100%;">
                    <h3 class="ilan-baslik"><i class='bx bx-car'></i> ${a.marka} ${a.model}</h3>
                    <div style="font-weight:bold; color:#f39c12;"><i class='bx bx-timer'></i> Aktif Kirada</div>
                </div>
                <div style="width: 100%; display: flex; justify-content: space-between; margin-top:10px; font-size:14px;">
                    <span>Günlük Getiri: <b style="color:#27ae60;">+${paraFormatla(gunlukKira)} ₺</b></span>
                    <span>Değeri: ${paraFormatla(a.fiyat)} ₺</span>
                </div>
                <button class="btn btn-kirmizi" style="margin-top:15px; width: 100%;" onclick="kiradanCek(${a.id})"><i class='bx bx-arrow-back'></i> Vitrine (Garaja) Çek</button>
            </div>`;
        });
    }
}

// OYUNU KIRAN ANA FONKSİYON (GÜN ATLARKEN HATA VERİYORDU)
function rentACarGelirVeRiskYonetimi() {
    if(rentACarFilosu.length === 0) return;
    let gunlukToplamKira = 0;
    
    for(let i = rentACarFilosu.length - 1; i >= 0; i--) {
        let a = rentACarFilosu[i];
        let kira = Math.floor(a.fiyat * 0.005);
        gunlukToplamKira += kira;
        
        // Risk 1: Müşteri araba ile kaza yapabilir (Günlük %2 İhtimal)
        if(Math.random() < 0.02) {
            a.hasarli = true;
            let hasarMasrafi = Math.floor(a.fiyat * (Math.random() * 0.15 + 0.05));
            a.tamirMasrafi += hasarMasrafi;
            a.fiyat = Math.floor(a.fiyat * 0.85); // Aracın değeri düşer
            
            rentACarFilosu.splice(i, 1);
            garaj.push(a); // Kaza yapan araç hasarlı olarak garaja döner
            
            oyunSesi('hata');
            setTimeout(() => { ozelUyari(`🚨 RENT A CAR KAZASI!\nKiradaki ${a.marka} aracı kaza yaptı. Ağır hasarlı olarak garaja çekildi!`, "hata"); }, 1500);
        } 
        // Risk 2: Trafik Cezası yenebilir (Günlük %5 İhtimal)
        else if (Math.random() < 0.05) {
            let ceza = Math.floor(Math.random() * 2000) + 500;
            paramiz -= ceza;
            toplamGider += ceza;
            setTimeout(() => { ozelUyari(`✉️ Ceza Makbuzu\nKiradaki ${a.marka} araç HGS ihlali yaptı. Para senden kesildi: -${ceza} ₺`, "bilgi"); }, 2000);
        }
    }
    
    // Kazanılan kirayı kasaya ekle
    if(gunlukToplamKira > 0) {
        paramiz += gunlukToplamKira;
        toplamGelir += gunlukToplamKira;
    }
}
