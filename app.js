function ozelUyari(mesaj, tip = 'bilgi') {
    try {
        ...
        if (tip === 'hata') { icerik.classList.add('uyari-hata'); ikon.innerText = '❌'; baslik.innerText = 'Hüsran!'; baslik.style.color = '#d63031'; } 
...
// V4.0 EKSPERTİZ SİMETRİSİ (TAVAN VE ÖN 3 PARÇA ETKİSİ)
function ekspertizUret() {
    ...
    // V4.0: SİMETRİ KONTROLÜ
    let agirHasarSebebi = "";
    if (ekspertiz['tavan'] === 'degisen' || ekspertiz['tavan'] === 'boyali') {
        hasarPuan += 25; // Tavan işlemli, takla atmış!
        agirHasarSebebi = "Tavan İşlemli (Taklalı)";
    }
    if (ekspertiz['kaput'] !== 'orijinal' && ekspertiz['solOnCamurluk'] !== 'orijinal' && ekspertiz['sagOnCamurluk'] !== 'orijinal') {
        hasarPuan += 20; // Ön 3 parça değişik!
        agirHasarSebebi = agirHasarSebebi ? agirHasarSebebi + " ve Ön 3 Parça" : "Ön 3 Parça İşlemli (Önden Kazalı)";
    }
...
function aciklamaUret(ekspertizPuan, km, marka, saticiTipi, agirHasarSebebi) {
    if (saticiTipi === "Galeri") {
        ...
        if (agirHasarSebebi) return `Aracın kazası belası yoktur, keyfe keder boyattım. Ufak bi trameri var. Alıcısına hayırlı olsun.`; // Sahibinden yalanı
...
// V4.0: GÜMRÜK, ŞEHİR VE SATICI TİPİ
function rastgeleArabaUret() {
    ...
    let gumrukAraci = false;
    let gumrukKalanGun = 0;

    // Gümrük İhtimali (%10)
    if (Math.random() < 0.10) {
        gumrukAraci = true;
        saticiTipi = "Gümrük Bakanlığı";
        sehir = "Edirne (Gümrük)";
        fiyat = Math.floor(fiyat * 0.65); // %35 Ucuz!
        gumrukKalanGun = 5; // 5 gün kilitli kalır
    }

    // Galeriler biraz daha pahalı, Sahibinden biraz daha ucuz satar
    if (saticiTipi === "Galeri") fiyat = Math.floor(fiyat * 1.05);
    if (saticiTipi === "Sahibinden") fiyat = Math.floor(fiyat * 0.95);

    let takasFiyati = Math.floor(fiyat * 1.08); // Takas fiyatı nakitten %8 pahalıdır
    ...
    return { 
        ...
        agirHasarSebebi: ekspertizVerisi.agirHasarSebebi,
...
function piyasayiYenile() { 
    arabalar = []; for(let i = 0; i < 6; i++) { arabalar.push(rastgeleArabaUret()); } // 6 araba oldu
    aktifEkraniYenile();
}
...
function sonrakiGun() {
    try {
        ...
        if (gun % 30 === 0) { 
            ...
            oyunSesi('hata'); ozelUyari(`📅 Ay sonu! Giderler toplamı ${toplamAylikGider.toLocaleString('tr-TR')} ₺ kasadan çekildi.`, "bilgi"); 
        }
                ...
                if (araba.tamirDurumu <= 0) { 
                    ...
                    if (araba.muayenede) {
                        ...
                        if (araba.modifiyeler.includes("Stage 1 Yazılım & Egzoz") && Math.random() < 0.80) {
                            let ceza = 15000; if(paramiz >= ceza) { paramiz -= ceza; toplamGider += ceza; araba.muayeneVar = true; oyunSesi('hata'); ozelUyari(`🚨 Abartı egzoz yüzünden araç muayeneden kaldı. Söktürmek için ${ceza.toLocaleString('tr-TR')} ₺ harcadın ama belgeyi aldın.`, "hata"); } 
                            else { araba.tamirDurumu = 1; araba.muayenede = true; ozelUyari("Aracın egzozdan muayeneden kaldı. Paran olmadığı için istasyonda yatıyor!", "hata"); }
...
// ==========================================
// V4.0 YENİ PAZAR VE SATIN ALMA EKRANLARI
// ==========================================
function arabalariEkranaGetir() { 
    ...
    // F5 Yenileme Butonu
    liste.innerHTML += `
        <div style="grid-column: 1 / -1; background: #fffdf0; border: 1px dashed #f1c40f; padding: 15px; border-radius: 10px; text-align: center; margin-bottom: 15px;">
            <p style="margin:0 0 10px 0; color: #d35400; font-size: 14px;">Piyasadaki fırsatları kaçırma, yeni araçları görmek için siteyi yenile!</p>
            <button class="btn btn-turuncu" style="width: auto; padding: 10px 30px; margin: 0;" onclick="f5At()">🔄 Piyasayı Yenile (500 ₺)</button>
        </div>
    `;
    ...
    arabalar.forEach(a => { 
        ...
        liste.innerHTML += `
            <div class="ilan-karti" style="${a.gumrukAraci ? 'border: 2px solid #8e44ad;' : ''}">
                <div class="araba-foto"><img src="${a.gorsel}" style="width:100%; height:100%; object-fit:cover; border-radius:8px;"></div>
                <div class="ilan-detay">
                    <h3 class="ilan-baslik">${a.marka} ${a.model}</h3>
                    <div class="ilan-ozellikler"><span>🗓️ <strong>${a.yil}</strong></span><span>🛣️ <strong>${a.km.toLocaleString('tr-TR')}</strong> KM</span></div>
                    <div class="ilan-durum" style="margin-top: 5px;">${saticiBadge} ${sehirBadge} <br> <div style="margin-top:5px;">${muayeneBadge} ${hasarMetni}</div></div>
                </div>
                <div class="ilan-sag-taraf">
                    <div class="ilan-fiyat">${a.fiyat.toLocaleString('tr-TR')} ₺</div>
                    <div style="font-size:11px; color:#636e72; margin-bottom:8px; text-align:right;">Takas Fiyatı: ${a.takasFiyati.toLocaleString('tr-TR')} ₺</div>
...
function f5At() {
    ...
    paramiz -= 500; toplamGider += 500;
    oyunSesi('kasa'); piyasayiYenile(); ekraniGuncelle(); oyunuKaydet();
}
...
function ilanDetayEkraniAc(arabaId) { 
    ...
    try {
        ...
        let agirHasarUyariHTML = a.agirHasarSebebi !== "" ? `<div style="background:#ffcccc; color:#c0392b; padding:10px; border-radius:8px; margin-bottom:15px; font-weight:bold; font-size:13px;">🚨 UYARI: Bu araçta ${a.agirHasarSebebi} var! Piyasada satarken çok zorlanırsın.</div>` : "";
...
function saticiAra(id) { 
    ...
    setTimeout(() => { 
        ...
        // Gümrük aracıysa farklı diyalog
        if (a.gumrukAraci) {
            document.getElementById('tel-diyalog').innerText = `"Burası Edirne Gümrük Müdürlüğü. ${a.fiyat.toLocaleString('tr-TR')} TL ödemeyi yaparsanız işlemleri başlatırız. Aracın 5 gün çıkış izni süresi vardır."`; 
            document.getElementById('tel-aksiyonlar').innerHTML = `
                <button class="btn btn-yesil" onclick="telSatinAl(${a.id}, 0)" style="padding: 15px; font-size: 16px;">Ücreti Öde ve İşlemleri Başlat</button>
            `;
            document.getElementById('tel-aksiyonlar').style.display = 'flex'; 
            return;
        }

        let baslangicDiyalog = a.sehir === "İstanbul" ? `"Alo buyur kardeşim. Fiyat nakit ${a.fiyat.toLocaleString('tr-TR')} TL, takas fiyatım ${a.takasFiyati.toLocaleString('tr-TR')} TL."` : `"Alo buyur kardeşim. Ben ${a.sehir}'dayım. Fiyatım ${a.fiyat.toLocaleString('tr-TR')} TL. Araba dediğim gibidir."`;
        ...
        if (a.sehir === "İstanbul") {
            aksiyonButonlari = `
                ...
                <button class="btn btn-mavi" onclick="takasEkraniAc(${a.id})" style="padding: 15px; font-size: 16px;">🔄 Kendi Arabamı Takasa Ver</button>
...
function telPazarlikYap(id) { 
    ...
    // Galerici zor pazarlık yapar, Sahibinden daha kolay kırılır
    ...
    if (Math.random() < pazarIhtimali) { 
        ...
        a.takasFiyati = Math.floor(a.fiyat * 1.08);
        oyunSesi('kasa'); 
        document.getElementById('tel-diyalog').innerHTML = `"Hadi senin canın sağolsun kardeşim, nakit ${a.fiyat.toLocaleString('tr-TR')} TL olsun."`; 
        ...
        oyunSesi('hata'); document.getElementById('tel-diyalog').innerHTML = `"Biz esnafız kardeşim, dip rakam budur. Ölücülerle işim olmaz!"`; 
        arabalar = arabalar.filter(x => x.id !== id); aktifEkraniYenile(); 
    } 
}

// V4.0 ŞEHİR DIŞI EKSPERTİZ HÜSRANI RİSKİ
function sehirDisinaGit(id) {
    if (paramiz < 2500) { ozelUyari("Uçak bileti alacak 2.500 TL'niz yok!", "hata"); return; }
    paramiz -= 2500; toplamGider += 2500; ekraniGuncelle();
    ...
    setTimeout(() => { 
        ...
        let yalanIhtimali = a.saticiTipi === "Sahibinden" ? 0.35 : 0.05; // Sahibinden 35% yalan söyler!
        
        if (Math.random() < yalanIhtimali && !a.hasarli) {
            // ŞOK! Adam yalan söylemiş
            oyunSesi('hata');
            a.hasarli = true; a.tamirMasrafi = Math.floor(a.fiyat * 0.15); // Hasarlıya düştü
            document.getElementById('tel-aranan-kisi').innerText = `🚨 BÜYÜK ŞOK!`; 
            document.getElementById('tel-diyalog').innerHTML = `<span style="color:#e74c3c; font-weight:bold;">Adamın telefonda "Hatasız" dediği araç 3 parça değişenli çıktı! Sinirle uçağa binip geri döndün. 2.500 ₺ yol masrafı çöpe gitti.</span>`; 
...
function takasEkraniAc(id) {
    ...
    garaj.forEach(benimArabam => {
        // Sadece sağlam, sanayide olmayan ve gümrükte kilitli olmayanlar takasa girebilir
        if(benimArabam.tamirDurumu === 0 && benimArabam.muayeneVar && benimArabam.gumrukKalanGun === 0) {
            uygunAracVarMi = true;
            let fark = karsiAraba.takasFiyati - benimArabam.fiyat;
            ...
            lst.innerHTML += `
                <div class="teklif-karti">
                    <div style="text-align:left;">
                        <b>${benimArabam.marka} ${benimArabam.model}</b><br>
                        Senin Aracın: ${benimArabam.fiyat.toLocaleString('tr-TR')} ₺ <br>
                        Karşının Takas Fiyatı: ${karsiAraba.takasFiyati.toLocaleString('tr-TR')} ₺ <br>
                        <b>${farkMetni}</b>
                    </div>
                    <button class="btn btn-mavi" style="width:auto; margin:0;" onclick="takasiTamamla(${benimArabam.id}, ${karsiAraba.id}, ${fark})">Takasla</button>
                </div>
            `;
        }
    });

    if(!uygunAracVarMi) { lst.innerHTML = `<p style="color:#d63031; text-align:center; font-weight:bold;">Garajında takasa verebileceğin uygun araç yok!</p>`; }
    document.getElementById('takas-secim-modal').style.display = 'block';
}

function takasiTamamla(benimArabaId, karsiArabaId, fark) {
    if (fark > 0 && paramiz < fark + noterUcreti + sigortaVeMtvUcreti) { ozelUyari("Üste verecek nakit paran yok!", "hata"); return; }
    
    const karsiAraba = arabalar.find(x => x.id === karsiArabaId); 
    
    if (fark > 0) { 
        paramiz -= (fark + noterUcreti + sigortaVeMtvUcreti); toplamGider += (fark + noterUcreti + sigortaVeMtvUcreti); 
    } else { 
        paramiz += (Math.abs(fark) - noterUcreti - sigortaVeMtvUcreti); toplamGelir += Math.abs(fark); toplamGider += (noterUcreti + sigortaVeMtvUcreti); 
    }

    garaj = garaj.filter(x => x.id !== benimArabaId); // Kendi aracımı sildim
    karsiAraba.fiyat = karsiAraba.takasFiyati; // Yeni aracımın maliyeti takas fiyatı oldu
    garaj.push(karsiAraba); // Yeni aracı garaja aldım
    arabalar = arabalar.filter(x => x.id !== karsiArabaId); // İlandan düştü
    toplamSatilanArac++;

    oyunSesi('kasa'); modaliKapat('takas-secim-modal'); aktifEkraniYenile(); oyunuKaydet();
    ozelUyari(`🔄 Takas Başarıyla Gerçekleşti!\nEski aracını verip ${karsiAraba.marka} aracını garaja çektin.`, "basari");
}
...
function telSatinAl(id, ekMasraf) { 
    ...
    if (paramiz >= t) { 
        ...
        if(a.gumrukAraci) { ozelUyari(`🇪🇺 Gümrük Aracı Alındı!\nAraç garaja çekildi ancak ÖTV ve ruhsat işlemleri için ${a.gumrukKalanGun} gün boyunca rehin kalacak.`, "basari"); }
        else { ozelUyari(`Araç garaja çekildi!\nEk Masraflar Dahil Toplam Çıkan: ${t.toLocaleString('tr-TR')} ₺`, "basari"); }
    } else { 
        oyunSesi('hata'); document.getElementById('tel-diyalog').innerHTML = `"Para çıkışmıyor kardeşim, bizi mi oyalıyorsun!"`; document.getElementById('tel-aksiyonlar').style.display = 'none'; 
    } 
}
...
// ... (Bütün diğer tamir, kasko, personel, banka, ihale, canlı yayın fonksiyonları önceki sürümdeki gibi sorunsuz duruyor, onlara dokunmuyorum) ...
function muayeneyeSok(id) { const a = garaj.find(x => x.id === id); if(a.hasarli) { ozelUyari("Araç ağır hasarlıyken muayeneden geçemez! Önce sanayide kaportayı toplat.", "hata"); return; } if(paramiz < 3500) { ozelUyari("TÜVTÜRK harcı için 3.500 TL lazım!", "hata"); return; } paramiz -= 3500; toplamGider += 3500; a.tamirDurumu = 1; a.muayenede = true; a.teklifler = []; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari("Araç TÜVTÜRK istasyonuna bırakıldı. Yarın muayeneden çıkacak.", "basari"); }
...
function rentACarGelirVeRiskYonetimi() { if (rentACarFilosu.length === 0) return; let kazanilan = 0; for (let i = rentACarFilosu.length - 1; i >= 0; i--) { let a = rentACarFilosu[i]; if(!a.gunlukKiraBedeli) a.gunlukKiraBedeli = Math.floor(a.fiyat * 0.005); let risk = Math.random(); if (risk < 0.02) { rentACarFilosu.splice(i, 1); if(a.kaskolu) { paramiz += a.fiyat; toplamGelir += a.fiyat; oyunSesi('kasa'); ozelUyari(`🚨 Aracın Çalındı! Ancak 🛡️ KASKO aracın güncel piyasa değeri olan ${a.fiyat.toLocaleString('tr-TR')} ₺ hesabına yatırıldı.`, "basari"); } else { oyunSesi('hata'); ozelUyari(`🚨 ŞOK! Kiradaki ${a.marka} aracın çalındı ve kaskosu yoktu! Koca araba çöp oldu.`, "hata"); } } else if (risk < 0.07) { rentACarFilosu.splice(i, 1); if(a.kaskolu) { paramiz += a.fiyat; toplamGelir += a.fiyat; oyunSesi('kasa'); ozelUyari(`🚨 Kaza Haberi! Müşteri aracı pert etti ancak 🛡️ KASKO aracın tam bedeli olan ${a.fiyat.toLocaleString('tr-TR')} ₺ tutarını sana nakit ödedi.`, "basari"); } else { a.hasarli = true; a.tamirMasrafi = Math.floor(a.fiyat * 0.20); a.fiyat = Math.floor(a.fiyat * 0.70); garaj.push(a); oyunSesi('hata'); ozelUyari(`🚨 KAZA! Müşteri ${a.marka} ile kaza yapmış! Çekiciyle garaja atıldı. Kaskosu olmadığı için tüm zarar sende.`, "hata"); } } else if (risk < 0.15) { if (a.kaskolu) { kazanilan += a.gunlukKiraBedeli; } else { let ceza = Math.floor(a.gunlukKiraBedeli * 2); paramiz -= ceza; toplamGider += ceza; kazanilan += a.gunlukKiraBedeli; ozelUyari(`⚠️ Kiradaki ${a.marka} hor kullanılmış. Kasko olmadığı için ${ceza} ₺ masraf sana kaldı.`, "bilgi"); } } else { kazanilan += a.gunlukKiraBedeli; a.km += Math.floor(Math.random() * 200) + 50; } } if (kazanilan > 0) { paramiz += kazanilan; toplamGelir += kazanilan; oyunSesi('kasa'); } }
...
function euroAl(miktar) { let maliyet = miktar * euroKuru; if (paramiz >= maliyet) { paramiz -= maliyet; euroBakiye += miktar; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Euro alındı.`, 'basari'); } else { oyunSesi('hata'); ozelUyari("Para yok!", "hata"); } } function euroBozdur(miktar) { if (euroBakiye >= miktar) { paramiz += miktar * euroKuru; euroBakiye -= miktar; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Euro bozduruldu.`, 'basari'); } else { oyunSesi('hata'); ozelUyari("Euro yok!", "hata"); } } function tumEurouBozdur() { if(euroBakiye > 0) euroBozdur(euroBakiye); }
