...
function oyunuYukle() {
    ...
    if (eskiKayit && eskiKayit.galeriAdi) {
        ...
        rentACarFilosu.forEach(a => { if(a.kaskolu === undefined) a.kaskolu = false; });
...
// ==========================================
// 3. EKRAN GÜNCELLEME MOTORU (KIRILMAZ)
// ==========================================
...
function tramerUret(hasarPuan, tavanHasarliMi) {
    let sasiNo = "WBA" + Math.random().toString(36).substring(2, 8).toUpperCase() + "***"; let mesaj = `Kayıtlarımıza göre <b>${sasiNo}</b> şasi numaralı araçta `;
    if (hasarPuan === 0) return mesaj + `<b>HASAR KAYDI BULUNMAMIŞTIR.</b>`;
    let kazaSayisi = Math.floor(Math.random() * 3) + 1; let toplamTramer = hasarPuan * (Math.floor(Math.random() * 15000) + 10000); 
    if (tavanHasarliMi || hasarPuan > 15) return mesaj + `<b>AĞIR HASAR KAYDI (ÇARPMA)</b> bulunmuştur. Toplam: <b>${toplamTramer.toLocaleString('tr-TR')} TL</b>.`;
    return mesaj + `<b>${kazaSayisi} adet</b> kazaya karışmıştır. Toplam: <b>${toplamTramer.toLocaleString('tr-TR')} TL</b>.`;
}
function aciklamaUret(ekspertizPuan, km, marka) { if (ekspertizPuan === 0 && km < 50000) return `Kapalı garaj arabasıdır. Nokta hatasızdır.`; if (ekspertizPuan === 0) return `Motoru kusursuzdur. Yaşına göre ufak tefek çizikleri var.`; if (ekspertizPuan < 5) return `Araçta sadece temizlik boyaları mevcuttur. Şase işlemsizdir.`; return `Araç ağır hasar kayıtlıdır. Takıntısı olan aramasın.`; }
function rastgeleArabaUret() {
    ...
    return { id: idSayaci++, marka: sablon.marka, model: sablon.model, yil: yil, km: Math.floor(km), fiyat: Math.floor(fiyat), hasarli: agirHasarliMi, tamirMasrafi: agirHasarliMi ? Math.floor(fiyat * 0.1) : 0, modifiyeler: [], gorsel: sablon.gorsel, ekspertiz: ekspertizVerisi.detay, ilanAciklamasi: aciklamaUret(ekspertizVerisi.puan, km, sablon.marka), teklifler: [], telefon: '05' + Math.floor(Math.random() * 90000000 + 10000000), tramer: tramerUret(ekspertizVerisi.puan, agirHasarliMi), tamirDurumu: 0, muayeneVar: muayeneDurumu, muayenede: false, kaskolu: false };
}
...
function ekonomiOlayiTetikle() { 
    ...
    if (sans < 0.05 && piyasaDurumu !== "Kriz") { piyasaDurumu = "Kriz"; piyasaCarpani = 0.85; euroKuru *= 1.10; oyunSesi('hata'); ozelUyari("📉 FLAŞ HABER: Kredi faizleri uçtu, piyasa kilitlendi! Araç fiyatları düştü, Euro fırladı!", "hata"); } 
    else if (sans > 0.95 && piyasaDurumu !== "Canli") { piyasaDurumu = "Canli"; piyasaCarpani = 1.20; euroKuru *= 0.95; oyunSesi('kasa'); ozelUyari("📈 FLAŞ HABER: Devletten muazzam kredi kampanyası! Millet galeriye akın ediyor!", "basari"); } 
...
// ==========================================
// 5. GÜN ATLATMA VE ZAMAN DÖNGÜSÜ
// ==========================================
function sonrakiGun() {
    try {
                ...
                if (araba.tamirDurumu <= 0) { 
                    ...
                    if (araba.muayenede) {
                        ...
                        if (araba.modifiyeler.includes("Stage 1 Yazılım & Egzoz") && Math.random() < 0.80) {
                            let ceza = 15000;
                            if(paramiz >= ceza) { paramiz -= ceza; toplamGider += ceza; araba.muayeneVar = true; oyunSesi('hata'); ozelUyari(`🚨 Aracındaki abartı egzoz yüzünden muayeneden kaldı. Söktürmek için ${ceza.toLocaleString('tr-TR')} ₺ harcadın ama belgeyi aldın.`, "hata"); } 
                        ...
                        } else { araba.muayeneVar = true; oyunSesi('kasa'); ozelUyari("✅ Araç TÜVTÜRK'ten kusursuz geçti! Satışa hazır.", "basari"); }
                    } else {
...
// ==========================================
// 6. GARAJ, PAZAR VE SATIŞ SİSTEMLERİ
// ==========================================
...
function ilanDetayEkraniAc(arabaId) { 
    ...
    try {
        ...
        document.getElementById('detay-tramer-btn').onclick = function() { if(paramiz<150){ ozelUyari("Paranız yetersiz!","hata"); return; } paramiz-=150; oyunSesi('kasa'); document.getElementById('tramer-mesaj-icerik').innerHTML=a.tramer; document.getElementById('tramer-modal').style.display='block'; }; 
...
function saticiAra(id) { 
    ...
    document.getElementById('tel-aranan-kisi').innerText = `Satıcı Aranıyor`; document.getElementById('tel-diyalog').innerText = "Dıt... Dıt..."; document.getElementById('tel-aksiyonlar').style.display = 'none'; document.querySelector('.telefon-ekrani').classList.add('caliyor'); document.getElementById('telefon-modal').style.display = 'block'; 
    setTimeout(() => { 
        document.querySelector('.telefon-ekrani').classList.remove('caliyor'); document.getElementById('tel-diyalog').innerText = `"Alo buyur kardeşim. Fiyat ${a.fiyat.toLocaleString('tr-TR')} TL."`; document.getElementById('tel-aksiyonlar').style.display = 'flex'; 
        ...
        document.getElementById('tel-pazarlik-btn').onclick = function() { 
            ...
            if (Math.random() > 0.40) { a.fiyat = Math.floor(a.fiyat * (1 - ((Math.floor(Math.random()*8)+3)/100))); oyunSesi('kasa'); document.getElementById('tel-diyalog').innerHTML = `"Hadi ${a.fiyat.toLocaleString('tr-TR')} TL olsun."`; document.getElementById('tel-aksiyonlar').style.display = 'flex'; document.getElementById('tel-pazarlik-btn').style.display = 'none'; } 
            else { oyunSesi('hata'); document.getElementById('tel-diyalog').innerHTML = `"Ölücülerle işim olmaz!"`; arabalar = arabalar.filter(x => x.id !== id); aktifEkraniYenile(); } 
        }; 
    }, 2000); 
}
...
function hakanAbiAra(id) { 
    ...
    setTimeout(() => { 
        document.querySelector('.telefon-ekrani').classList.remove('caliyor'); let ind = Math.floor(a.fiyat * 0.60); document.getElementById('tel-diyalog').innerHTML = `"Adam ${ind.toLocaleString('tr-TR')} TL'ye bırakıyor."`; document.getElementById('tel-aksiyonlar').style.display = 'flex'; document.getElementById('tel-pazarlik-btn').style.display = 'none'; document.getElementById('tel-satin-al-btn').onclick = function() { a.fiyat = ind; hakanAbiSonKullanim = gun; telSatinAl(a.id); }; 
    }, 2500); 
}

function telSatinAl(id) { 
    const a = arabalar.find(x => x.id === id); let t = a.fiyat + noterUcreti + sigortaVeMtvUcreti; 
    if (paramiz >= t) { paramiz -= t; a.tamirDurumu = 0; garaj.push(a); arabalar = arabalar.filter(x => x.id !== id); oyunSesi('kasa'); telefonuKapat(); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Araç garaja çekildi!`, "basari"); } 
    else { oyunSesi('hata'); document.getElementById('tel-diyalog').innerHTML = `"Para çıkışmıyor kardeşim!"`; document.getElementById('tel-aksiyonlar').style.display = 'none'; } 
}
...
function muayeneyeSok(id) {
    ...
    paramiz -= 3500; toplamGider += 3500; a.tamirDurumu = 1; a.muayenede = true; a.teklifler = [];
    oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari("Araç TÜVTÜRK istasyonuna bırakıldı. Yarın muayeneden çıkacak.", "basari");
}
...
function kirayaVer(arabaId) { 
    ...
    if (!araba.muayeneVar) { ozelUyari("Muayenesiz (Çekme Belgeli) aracı kiraya veremezsin!", "hata"); return; }
    if (araba.hasarli || araba.tamirDurumu > 0) { ozelUyari("Hasarlı aracı kiraya veremezsin!", "hata"); return; } 
    araba.gunlukKiraBedeli = Math.floor(araba.fiyat * 0.005); rentACarFilosu.push(araba); garaj.splice(arabaIndex, 1); 
...
function rentACarGelirVeRiskYonetimi() { 
    ...
    for (let i = rentACarFilosu.length - 1; i >= 0; i--) { 
        let a = rentACarFilosu[i]; if(!a.gunlukKiraBedeli) a.gunlukKiraBedeli = Math.floor(a.fiyat * 0.005); let risk = Math.random(); 
        if (risk < 0.02) { 
            rentACarFilosu.splice(i, 1); 
            if(a.kaskolu) { paramiz += a.fiyat; toplamGelir += a.fiyat; oyunSesi('kasa'); ozelUyari(`🚨 Araç Çalındı! Ancak 🛡️ KASKO aracın bedelini ödedi.`, "basari"); } else { oyunSesi('hata'); ozelUyari(`🚨 ŞOK! Kiradaki ${a.marka} aracın çalındı ve kaskosu yoktu!`, "hata"); }
        } else if (risk < 0.07) { 
            rentACarFilosu.splice(i, 1); 
            if(a.kaskolu) { paramiz += a.fiyat; toplamGelir += a.fiyat; oyunSesi('kasa'); ozelUyari(`🚨 Kaza Haberi! Müşteri aracı pert etti ancak 🛡️ KASKO bedelini ödedi.`, "basari"); } else { a.hasarli = true; a.tamirMasrafi = Math.floor(a.fiyat * 0.20); a.fiyat = Math.floor(a.fiyat * 0.70); garaj.push(a); oyunSesi('hata'); ozelUyari(`🚨 KAZA! Müşteri kaza yaptı! Çekiciyle garaja atıldı. Kasko yok zarar sende.`, "hata"); }
        } else if (risk < 0.15) { 
            if (a.kaskolu) { kazanilan += a.gunlukKiraBedeli; } else { let ceza = Math.floor(a.gunlukKiraBedeli * 2); paramiz -= ceza; toplamGider += ceza; kazanilan += a.gunlukKiraBedeli; ozelUyari(`⚠️ Kiradaki ${a.marka} hor kullanılmış. Kasko olmadığı için ${ceza} ₺ masraf sana kaldı.`, "bilgi"); }
...
// ==========================================
// 8. DÖVİZ, SENET, İSTATİSTİK VE PERSONEL
// ==========================================
...
// ==========================================
// 9. SOSYAL MEDYA, İHALE VE CANLI YAYIN
// ==========================================
