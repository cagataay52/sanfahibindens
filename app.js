// ==========================================
// 1. GENEL AYARLAR VE DEĞİŞKENLER (V7.5 KUSURSUZ + AYARLAR)
// ==========================================
...
// Yeni Eklenen Araçlarla Birlikte Tam Liste
const aracSablonlari = [ 
    ...
    { marka: "Citroën", model: "C5 Aircross", tabanFiyat: 2050000, gorsel: "img/c5-aircross.jpg" },
    { marka: "Volkswagen", model: "Tiguan", tabanFiyat: 2450000, gorsel: "img/tiguan.jpg" },
    { marka: "Volkswagen", model: "Passat", tabanFiyat: 2200000, gorsel: "img/passat.jpg" },
    { marka: "Volkswagen", model: "Jetta", tabanFiyat: 1250000, gorsel: "img/jetta.jpg" },
    { marka: "Fiat", model: "Linea", tabanFiyat: 650000, gorsel: "img/linea.jpg" },
    { marka: "Fiat", model: "Albea", tabanFiyat: 450000, gorsel: "img/albea.jpg" },
    { marka: "Fiat", model: "Egea Cross", tabanFiyat: 1350000, gorsel: "img/egea-cross.jpg" },
    { marka: "Mercedes", model: "A200 Sedan", tabanFiyat: 2650000, gorsel: "img/a200-sedan.jpg" },
    { marka: "Mercedes", model: "GLA 200", tabanFiyat: 2850000, gorsel: "img/gla200.jpg" },
    { marka: "Audi", model: "Q3", tabanFiyat: 3150000, gorsel: "img/audi-q3.jpg" }
];
...
function oyunSesi(tip) {
    if (!ayarlar.sesAcik) return; // Ayarlardan ses kapalıysa işlem yapma
    try {
...
function paraFormatla(miktar) {
    if(!ayarlar.kompaktPara) return miktar.toLocaleString('tr-TR');
    if(miktar >= 1000000) return (miktar / 1000000).toFixed(2).replace('.00', '') + "M";
    if(miktar >= 1000) return (miktar / 1000).toFixed(1).replace('.0', '') + "K";
    return miktar.toString();
}

function ayarDegistir(ayarAdi) {
    if(ayarAdi === 'ses') ayarlar.sesAcik = document.getElementById('ayar-ses').checked;
    if(ayarAdi === 'kompakt') { ayarlar.kompaktPara = document.getElementById('ayar-kompakt').checked; ekraniGuncelle(); }
    if(ayarAdi === 'hizliyenile') ayarlar.hizliYenile = document.getElementById('ayar-hizliyenile').checked;
    oyunuKaydet();
}

function ekraniGuncelle() {
    paramiz = Math.round(paramiz);
    euroBakiye = Math.round(euroBakiye);
...
function oyunuYukle() {
    ...
    if (eskiKayit && eskiKayit.galeriAdi) {
        ...
        euroKuru = eskiKayit.euroKuru || 38.50; euroBakiye = Math.round(eskiKayit.euroBakiye) || 0; senetler = eskiKayit.senetler || []; 
        ...
        let aSes = document.getElementById('ayar-ses'); if(aSes) aSes.checked = ayarlar.sesAcik;
        let aKomp = document.getElementById('ayar-kompakt'); if(aKomp) aKomp.checked = ayarlar.kompaktPara;
        let aHiz = document.getElementById('ayar-hizliyenile'); if(aHiz) aHiz.checked = ayarlar.hizliYenile;
        ...
        garaj.forEach(a => { 
            if(a.tamirDurumu === undefined) a.tamirDurumu = 0; if(a.muayeneVar === undefined) a.muayeneVar = true; if(a.muayenede === undefined) a.muayenede = false; if(a.gumrukKalanGun === undefined) a.gumrukKalanGun = 0; 
            if(a.vites === undefined) a.vites = "Otomatik"; if(a.yakit === undefined) a.yakit = "Benzin"; if(a.motor === undefined) a.motor = "1.6"; if(a.hp === undefined) a.hp = 110; if(!a.donanimlar) a.donanimlar = ["Cam Tavan", "Led Far"];
        });
...
function rastgeleArabaUret() {
    ...
    if (saticiTipi === "Galeri") fiyat = Math.round(fiyat * 1.05); if (saticiTipi === "Sahibinden") fiyat = Math.round(fiyat * 0.95);
    let takasFiyati = Math.round(fiyat * 1.08);

    let vitesler = ["Otomatik", "Manuel"]; let vites = vitesler[Math.floor(Math.random() * vitesler.length)];
    let yakitTipleri = ["Benzin", "Dizel"]; if (sablon.marka === "Toyota" || sablon.marka === "Citroën") yakitTipleri.push("Hibrit");
    ...
    let rehinBedeli = rehinliMi ? Math.round(fiyat * (Math.random() * 0.25 + 0.05)) : 0; 
...
document.addEventListener('touchend', function(e) { if(!ptrCekiliyor || !ptrAlan) return; if(ptrAlan.classList.contains('aktif')) { if(bakiyeYeterliMi(500) && !eHacizAktif) { f5At(); oyunSesi('dokun'); } else { ozelUyari("Limit yetersiz veya E-Haciz var!", "hata"); } setTimeout(() => { ptrAlan.classList.remove('aktif'); }, 500); } ptrCekiliyor = false; ptrBaslangicY = 0; });
...
function sonrakiGun() {
    try {
        ...
        if (krediler.length > 0) { 
            krediler = krediler.filter(k => { 
                paramiz -= k.taksit; 
                toplamGider += k.taksit; 
                k.kalanGun--; 
                krediNotu += 2; 
                if (k.kalanGun <= 0) {
                    krediNotu += 50; 
                    ozelUyari("🎉 Bir kredinin ödemesi bitti! Findeks notun fırladı.", "basari");
                    return false;
                }
                return true;
            }); 
...
function bankaEkraniniGuncelle() {
    ...
    let vb = document.getElementById('vergi-borcu-ekran'); if(vb) vb.innerText = Math.round(vergiBorcu).toLocaleString('tr-TR'); let hUyari = document.getElementById('e-haciz-uyari'); if(hUyari) hUyari.style.display = eHacizAktif ? 'block' : 'none';
...
function hisseModalHesapla() { if (aktifHisseIndex === -1) return; let h = borsa[aktifHisseIndex]; let adet = parseInt(document.getElementById('hisse-modal-adet').value) || 0; let tutar = Math.round(adet * h.fiyat); document.getElementById('hisse-modal-tutar').innerText = tutar.toLocaleString('tr-TR') + " ₺"; }
function hisseIslemOnayla() { 
    if (aktifHisseIndex === -1) return; 
    let h = borsa[aktifHisseIndex]; let adet = parseInt(document.getElementById('hisse-modal-adet').value) || 0; 
    if (adet <= 0) { ozelUyari("Geçerli adet girin.", "hata"); return; } 
    let tutar = Math.round(adet * h.fiyat); 
    if (aktifHisseIslemTipi === 'al') { 
        if(bakiyeYeterliMi(tutar)) { 
            paramiz -= tutar; toplamGider += tutar; 
            let toplamMaliyet = (h.portfoyAdet * h.maliyet) + tutar; h.portfoyAdet += adet; h.maliyet = toplamMaliyet / h.portfoyAdet; 
            oyunSesi('kasa'); modaliKapat('hisse-islem-modal'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`${adet} lot ${h.kod} alındı.`, "basari"); 
        } else { ozelUyari("Limit yetersiz!", "hata"); } 
    } else { 
        if(adet > h.portfoyAdet) { ozelUyari("O kadar hissen yok!", "hata"); return; } 
        paramiz += tutar; toplamGelir += tutar; h.portfoyAdet -= adet; if(h.portfoyAdet === 0) h.maliyet = 0; 
        oyunSesi('kasa'); modaliKapat('hisse-islem-modal'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`${adet} lot ${h.kod} satıldı.`, "basari"); 
    } 
}

function krediCek(tip) { if(eHacizAktif) { ozelUyari("Hesaplarında e-Haciz var!", "hata"); return; } let paketler = { 'esnaf': { miktar: 1000000, geriOdeme: 1200000, vade: 10, minNot: 500 }, 'arac': { miktar: 5000000, geriOdeme: 6500000, vade: 20, minNot: 1200 }, 'holding': { miktar: 25000000, geriOdeme: 35000000, vade: 30, minNot: 1600 } }; let p = paketler[tip]; if (krediNotu < p.minNot) { ozelUyari(`Reddedildi!\nNotunuzun en az ${p.minNot} olması gerekiyor.`, "hata"); return; } krediler.push({ tip: tip, miktar: p.miktar, toplamBorc: p.geriOdeme, taksit: Math.floor(p.geriOdeme/p.vade), kalanGun: p.vade }); paramiz += p.miktar; krediNotu -= 20; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`${p.miktar.toLocaleString('tr-TR')} ₺ kredi onaylandı!`, "basari"); }
function vadeliyeYatir() { if(eHacizAktif) { ozelUyari("e-Haciz var!", "hata"); return; } if(mevduat.aktif) { ozelUyari("Zaten aktif vadeli hesabın var!", "hata"); return; } let miktarStr = prompt(`Vadeliye kaç TL yatırmak istiyorsun?\n(10 Gün, %15 Getiri)\nNakit: ${paramiz.toLocaleString('tr-TR')} ₺`, "1000000"); let miktar = parseInt(miktarStr); if(isNaN(miktar) || miktar <= 0) return; if(paramiz < miktar) { ozelUyari("Kasanda o kadar nakit yok!", "hata"); return; } paramiz -= miktar; mevduat = { aktif: true, anapara: miktar, kalanGun: 10, faizOrani: 0.15 }; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`${miktar.toLocaleString('tr-TR')} ₺ bankaya %15 faizle kilitlendi.`, "basari"); }
function euroAl(miktar) { if(eHacizAktif) { ozelUyari("e-Haciz var!", "hata"); return; } let maliyet = Math.round(miktar * euroKuru); if (bakiyeYeterliMi(maliyet)) { paramiz -= maliyet; euroBakiye += miktar; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`${miktar.toLocaleString('tr-TR')} Euro alındı.`, 'basari'); } else { oyunSesi('hata'); ozelUyari("Limitiniz yetersiz!", "hata"); } } 
function euroBozdur(miktar) { if(eHacizAktif) { ozelUyari("e-Haciz var!", "hata"); return; } if (euroBakiye >= miktar) { let gelir = Math.round(miktar * euroKuru); euroBakiye -= miktar; paramiz += gelir; oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`${miktar.toLocaleString('tr-TR')} Euro bozduruldu.`, 'basari'); } else { oyunSesi('hata'); ozelUyari("Euro yok!", "hata"); } } 
...
function arabalariEkranaGetir() { 
    ...
    arabalar.forEach(a => { 
        ...
        liste.innerHTML += `<div class="ilan-karti" style="${a.gumrukAraci ? 'border: 2px solid #8e44ad;' : ''}"><div class="araba-foto"><img src="${a.gorsel}" style="width:100%; height:100%; object-fit:cover; border-radius:8px;"></div><div class="ilan-detay"><h3 class="ilan-baslik">${a.marka} ${a.model}</h3><div class="ilan-ozellikler"><span>🗓️ <strong>${a.yil}</strong></span><span>🛣️ <strong>${a.km.toLocaleString('tr-TR')}</strong> KM</span></div><div class="ilan-durum" style="margin-top: 5px;">${saticiBadge} ${sehirBadge} <br> <div style="margin-top:5px;">${muayeneBadge} ${hasarMetni}</div></div></div><div class="ilan-sag-taraf"><div class="ilan-fiyat">${paraFormatla(a.fiyat)} ₺</div><div style="font-size:11px; color:var(--text-muted); margin-bottom:8px; text-align:right;">Takas Fiyatı: ${paraFormatla(a.takasFiyati)} ₺</div><button class="btn btn-turuncu" onclick="ilanDetayEkraniAc(${a.id})"><i class='bx bx-search-alt'></i> İncele</button></div></div>`; 
    }); 
}
function f5At() { 
    if(eHacizAktif) { ozelUyari("Hesaplarında e-Haciz var! İşlem yapılamaz.", "hata"); return; } 
    if (!bakiyeYeterliMi(500)) { ozelUyari("Siteyi yenilemek için paran ve limitin yok!", "hata"); return; } 
    // Ayarlar (QoL): Hızlı Yenileme
    if (!ayarlar.hizliYenile) {
        if(!window.confirm("Piyasayı yenilemek için 500 ₺ kesilecek. Onaylıyor musun?")) return;
    }
    paramiz -= 500; toplamGider += 500; oyunSesi('kasa'); piyasayiYenile(); aktifEkraniYenile(); oyunuKaydet(); 
}
...
garajiEkranaGetir = function() { 
    ...
    if (garaj.length === 0) { if(blg) blg.style.display = 'block'; lst.classList.remove('showroom-grid'); } 
        ...
        garaj.forEach(a => { 
            ...
            lst.innerHTML += `<div class="showroom-karti"><div class="spotlight"></div><div class="showroom-img-kutu"><img src="${a.gorsel}" style="${a.gumrukKalanGun > 0 ? 'filter: grayscale(80%);' : ''}"></div><h3 class="showroom-baslik">${a.marka} ${a.model}</h3><div style="font-size:12px; color:var(--text-muted); margin-bottom:5px;">🗓️ ${a.yil} | 🛣️ ${a.km.toLocaleString('tr-TR')} KM</div><div class="showroom-fiyat">${paraFormatla(a.fiyat)} ₺</div>${ekstraBilgi}<div class="showroom-aksiyonlar">${aksiyonButonlari}</div></div>`; 
        }); 
    } 
};
...
function teklifiKabulEt(id, tId) { const a = garaj.find(x => x.id === id); const t = a.teklifler.find(x => x.id === tId); if((a.hasarli || gizliKusurluAraclar.includes(id)) && Math.random() < 0.25) { modaliKapat('teklif-modal'); eksperKriziBaslat(id, tId, "nakit", 0); return; } satisiTamamla(id, tId, "nakit", 0); }
function senetleSat(id, tId, sF) { const a = garaj.find(x => x.id === id); const t = a.teklifler.find(x => x.id === tId); if((a.hasarli || gizliKusurluAraclar.includes(id)) && Math.random() < 0.25) { modaliKapat('teklif-modal'); eksperKriziBaslat(id, tId, "senet", sF); return; } satisiTamamla(id, tId, "senet", sF); }
function eksperKriziBaslat(id, tId, tip, sF) { const a = garaj.find(x => x.id === id); let modal = document.getElementById('karar-modal'); document.getElementById('karar-baslik').innerText = "🚨 Oto Rapor Krizi!"; document.getElementById('karar-metin').innerHTML = `Müşteri aracı kendi eksperine soktu ve <b>gizli kusurları/hasarı</b> öğrendi! Çılgına döndü ve <i>"Benim geldiğim yol masrafını ve eksper paramı (5.000 ₺) ver, yoksa dükkanı dağıtırım!"</i> diye bağırıyor.`; let btn1 = document.getElementById('karar-btn-1'); let btn2 = document.getElementById('karar-btn-2'); btn1.innerText = "Öde ve Gönder (5.000 ₺)"; btn1.className = "btn btn-yesil"; btn1.onclick = function() { if(paramiz >= 5000) { paramiz -= 5000; toplamGider += 5000; } else { bankaBorcu += 5000; } a.teklifler = a.teklifler.filter(x => x.id !== tId); oyunSesi('hata'); modaliKapat('karar-modal'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari("Adamın parasını verip zararla gönderdin.", "bilgi"); }; btn2.innerText = "Kavga Et ve Kov"; btn2.className = "btn btn-kirmizi"; btn2.onclick = function() { haritaPuani -= 1.0; if(haritaPuani < 1.0) haritaPuani = 1.0; a.teklifler = a.teklifler.filter(x => x.id !== tId); oyunSesi('hata'); modaliKapat('karar-modal'); aktifEkraniYenile(); oyunuKaydet(); ozelUyari("Müşteriyle kavga ettin! Adam Google Haritalar'da sana 1 yıldız verdi.", "hata"); }; modal.style.display = 'block'; }
...
// ==========================================
// 9. PERSONEL, İK VE MORAL SİSTEMİ
// ==========================================
function personelEkraniGuncelle() { 
    let mBar = document.getElementById('moral-bar'); 
    let mTxt = document.getElementById('moral-text');
    ...
    roller.forEach(r => {
        ...
        if(b) {
            if(personeller[r]) { 
                b.innerHTML = `<button class="btn btn-kirmizi" onclick="personelKov('${r}')">İşten Çıkar</button> <div style="margin-top:5px; color:#27ae60; font-weight:bold;">✅ Çalışıyor</div>`; 
            } else { 
                b.innerHTML = `<button class="btn btn-yesil" onclick="personelIseAl('${r}')">İşe Al</button>`; 
            }
        }
    });
}
...
function personelIseAl(rol) { 
    ...
    personelMorali += 10; 
    if(personelMorali > 100) personelMorali = 100;
    
    oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); 
    ozelUyari(`Personel işe alındı!`, "basari"); 
}

function personelKov(rol) { 
    personeller[rol] = false; 
    personelMorali -= 15; 
    if (personelMorali < 0) personelMorali = 0;
    
    oyunSesi('hata'); aktifEkraniYenile(); oyunuKaydet(); 
    ozelUyari(`Personel kovuldu. Çalışanların morali bozuldu.`, "bilgi"); 
}

function primDagit() {
    if(!bakiyeYeterliMi(15000)) { ozelUyari("Prim dağıtacak bütçeniz yok!", "hata"); return; }
    ...
    oyunSesi('kasa'); aktifEkraniYenile(); oyunuKaydet(); 
    ozelUyari("💸 Personele 15.000 ₺ prim dağıtıldı. Yüzler gülüyor!", "basari");
}
...
function dukkanEkraniniGuncelle() { let isim=document.getElementById('dukkan-isim'); if(!isim)return; const bil = seviyeler[dukkanSeviyesi - 1]; isim.innerText = `${bil.isim} (Seviye ${bil.seviye})`; document.getElementById('dukkan-kapasite').innerText = bil.kapasite === 999 ? "Sınırsız" : bil.kapasite; const alan = document.getElementById('yukseltme-alani'); if (dukkanSeviyesi < seviyeler.length) { alan.style.display = 'block'; document.getElementById('yeni-seviye-isim').innerText = seviyeler[dukkanSeviyesi].isim; document.getElementById('yeni-seviye-fiyat').innerText = paraFormatla(seviyeler[dukkanSeviyesi].fiyat) + " TL"; } else { alan.innerHTML = `<h3 style="color: #27ae60;">Maksimum seviyedesin!</h3>`; } }
...
function istatistikleriGuncelle() { let st=document.getElementById('ist-satilan'); if(st) st.innerText=toplamSatilanArac; let gl=document.getElementById('ist-gelir'); if(gl) gl.innerText=paraFormatla(toplamGelir); let gd=document.getElementById('ist-gider'); if(gd) gd.innerText=paraFormatla(toplamGider); let nt=document.getElementById('ist-net'); if(nt){ let k=toplamGelir-toplamGider; nt.innerText=paraFormatla(k); nt.style.color=k<0?'#d63031':'#00b894'; } }
...
function sosyalEkraniGuncelle() { 
    ...
    if (!sosyalMedya.aktif) { if(k) k.style.display = 'block'; if(y) y.style.display = 'none'; } 
        ...
        document.getElementById('profil-takipci').innerText = Math.floor(sosyalMedya.takipci).toLocaleString('tr-TR'); document.getElementById('profil-gonderi-sayisi').innerText = sosyalMedya.gonderiler.length;
        ...
        else { sosyalMedya.gonderiler.forEach(p => { grid.innerHTML += `<div class="sm-post-item"><img src="${p.gorsel}"><div class="sm-post-overlay"><span>❤️ ${p.begeni.toLocaleString('tr-TR')}</span><span>💬 ${p.yorum.toLocaleString('tr-TR')}</span></div></div>`; }); }
...
function videoCekVePaylas(id) { 
    ...
    document.getElementById('post-sonuc-takipci').innerText = `+${kazanilanTakipci} Takipçi`; document.getElementById('post-yorumlar').innerHTML = `<div style="margin-bottom:5px;"><b>@sokak_tayfasi:</b> Ateş ediyor 🔥</div><div style="margin-bottom:5px;"><b>@otomanyak:</b> Fiyat nedir usta?</div><div><b>@${sosyalMedya.kullaniciAdi.replace('@','')}_fan:</b> Kral yine piyasayı belirlemişsin.</div>`; 
...
function dmKutusunuEkranaBas() { const k = document.getElementById('dm-kutusu'); if(!k) return; k.innerHTML = ''; if (dmKutusu.length === 0) { k.innerHTML = '<p style="color:var(--text-muted);">Gelen kutusu boş.</p>'; return; } dmKutusu.forEach((m, i) => { k.innerHTML += `<div class="ilan-karti"><div><b>📩 ${m.gonderen}</b><br>Teklif: ${paraFormatla(m.teklifFiyat)} ₺</div><div style="display:flex; gap:10px;"><button class="btn btn-yesil" onclick="dmTeklifKabul(${i}, ${m.arabaId})">Sat</button> <button class="btn btn-kirmizi" onclick="dmSil(${i})">Sil</button></div></div>`; }); }
