function ozelUyari(mesaj, tip = 'bilgi') {
    ...
    if (tip === 'hata') { 
        ...
        baslik.innerText = '✅ Başarılı'; 
        baslik.style.color = '#00b894'; 
    } else { 
        baslik.innerText = 'ℹ️ Bilgi'; 
...
// KAYIT SİSTEMİ (OTOMATİK ONARIM EKLENDİ)
...
function oyunuYukle() {
    ...
    // Eski bozuk kayıtları reddeder, Sosyal Medya sistemini arar.
    if (k && k.galeriAdi && k.sosyalMedya) {
    ...
    return false; // Veri bozuksa veya eskiyse yeni oyun başlatır
}
...
function ekonomiOlayiTetikle() {
    ...
    if (sans < 0.05 && piyasaDurumu !== "Kriz") { 
        ...
        ozelUyari("📉 FLAŞ HABER: Kredi faizleri uçtu, piyasa kilitlendi! Araç fiyatları düştü.", "hata"); 
    } 
    else if (sans > 0.95 && piyasaDurumu !== "Canli") { 
        piyasaDurumu = "Canli"; piyasaCarpani = 1.20; oyunSesi('kasa'); 
        ozelUyari("📈 FLAŞ HABER: Devletten kredi kampanyası! Fiyatlar fırladı!", "basari"); 
    } 
    else if (sans > 0.40 && sans < 0.45 && piyasaDurumu !== "Normal") { 
        piyasaDurumu = "Normal"; piyasaCarpani = 1.0; 
        ozelUyari("⚖️ Piyasa ateşini kaybetti, fiyatlar dengelendi.", "bilgi"); 
    }
...
function sonrakiGun() {
    ...
    if (gun % 30 === 0) {
        ...
        oyunSesi('hata'); 
        ozelUyari(`📅 Ay sonu geldi! Kira ve faturalar (${toplamAylikGider.toLocaleString('tr-TR')} ₺) ödendi.`, "bilgi");
    }

    if (bankaBorcu > 0) { 
        let faiz = Math.floor(bankaBorcu * 0.05); 
        bankaBorcu += faiz; 
        toplamGider += faiz; 
    ...
    if(Math.random() < 0.10) { 
        ...
        toplamGider += ceza; 
        oyunSesi('hata'); 
        ozelUyari(`Maliye denetimi! ${ceza.toLocaleString('tr-TR')} TL ceza yedin.`, 'hata'); 
    }
    
    ekonomiOlayiTetikle();

    // SPONSORLUK GELİRİ
    if (sosyalMedya.sponsorlukAktif) { 
        paramiz += 15000; 
        toplamGelir += 15000; 
    }
        ...
        if (araba.tamirDurumu > 0) {
            ...
            if (Math.random() < 0.4) {
                ...
                if(araba.modifiyeler.length >= 3 && Math.random() < 0.4) { 
                    tip = "Tayfa"; 
                    tFiyat = Math.floor(araba.fiyat * (Math.random() * 0.20 + 1.10)); 
                }
                else if (mTip < 0.2) { 
                    tip = "Olucu"; 
                    tFiyat = Math.floor(araba.fiyat * (Math.random() * 0.20 + 0.50)); 
                }
                else if (mTip < 0.45) { 
                    tip = "Takas"; 
                    takas = rastgeleArabaUret(); 
                    takas.fiyat = Math.floor(araba.fiyat * 0.6); 
    ...
    if (sosyalMedya.aktif && garaj.length > 0 && gun >= sosyalMedya.lincBitisGunu) {
        ...
        if (Math.random() < dmIhtimali) {
            ...
            dmKutusu.push({ 
                ...
                metin: `Araç duruyorsa nakit alıyorum.`, 
                teklifFiyat: teklif, 
                arabaId: sAraba.id 
            });
        }
    ...
    if(document.getElementById('sosyal-ekrani').style.display === 'block') {
...
function menuDegistir(menu) {
    ...
    if(window.innerWidth <= 768) { 
        document.querySelector('.sol-menu').classList.remove('acik'); 
        document.getElementById('mobil-menu-overlay').classList.remove('acik'); 
    }
}
...
function ekraniGuncelle() { 
    ...
    document.getElementById('kapasite-bilgi').innerText = garaj.length + "/" + (aracKapasitesi === 999 ? "Sınırsız" : aracKapasitesi); 
    document.getElementById('kapasite-bilgi').style.color = (garaj.length >= aracKapasitesi && aracKapasitesi !== 999) ? '#e74c3c' : '#0984e3'; 
}

// ARAÇ LİSTELEME VE DETAY (DÜZELTİLDİ)
function arabalariEkranaGetir() {
    const liste = document.getElementById('araba-listesi'); 
    liste.innerHTML = ''; 
    arabalar.forEach(a => {
        let hasarMetni = a.hasarli ? '<span class="etiket etiket-kirmizi">Ağır Hasarlı Olabilir</span>' : '<span class="etiket etiket-yesil">Ekspertiz Raporlu</span>';
        liste.innerHTML += `
            <div class="ilan-karti">
                ...
                <div class="ilan-detay">
                    ...
                    <div class="ilan-ozellikler"><span>🗓️ <strong>${a.yil}</strong></span><span>🛣️ <strong>${a.km.toLocaleString()}</strong> KM</span></div>
                    <div class="ilan-durum" style="margin-top:5px;">${hasarMetni}</div>
                </div>
                <div class="ilan-sag-taraf">
                    <div class="ilan-fiyat">${a.fiyat.toLocaleString()} ₺</div>
                    <button class="btn btn-turuncu" onclick="ilanDetayEkraniAc(${a.id})">🔍 İncele</button>
                </div>
            </div>`;
    });
}
...
function ilanDetayEkraniAc(id) {
    ...
    parcalar.forEach(p => { 
        ...
        if(div) div.className = `eks-parca ${araba.ekspertiz[p]}`; 
    });
    ...
    if(kg <= 0) { 
        hBtn.innerText = "👑 Hakan Abi'ye Çöktürt"; 
        hBtn.style.opacity = "1"; 
        hBtn.onclick = () => hakanAbiAra(araba.id); 
    } else { 
        hBtn.innerText = `👑 Hakan Abi Meşgul (${kg} Gün)`; 
        hBtn.style.opacity = "0.5"; 
        hBtn.onclick = () => ozelUyari("Hakan Abi meşgul!", "bilgi"); 
    }
...
function tramerSorgula(id) { 
    if(paramiz<150) return ozelUyari("Paranız yetersiz!", "hata"); 
    paramiz-=150; toplamGider+=150; 
    ekraniGuncelle(); oyunSesi('kasa'); 
...
function saticiAra(id) {
    ...
    document.getElementById('tel-aranan-kisi').innerText = `Satıcı Aranıyor (${a.telefon})`; 
    ...
    setTimeout(() => { 
        document.querySelector('.telefon-ekrani').classList.remove('caliyor'); 
        document.getElementById('tel-diyalog').innerText = `"Alo buyur kardeşim. Fiyat ${a.fiyat.toLocaleString()} TL."`; 
...
function telPazarlikYap(id) {
    ...
    document.getElementById('tel-aksiyonlar').style.display = 'none';
    if(Math.random() > 0.4) { 
        a.fiyat = Math.floor(a.fiyat * (1 - (Math.floor(Math.random()*8)+3)/100)); 
        oyunSesi('kasa'); 
        document.getElementById('tel-diyalog').innerHTML = `"Hadi senin hatrına ${a.fiyat.toLocaleString()} TL olsun gel al."`; 
        document.getElementById('tel-aksiyonlar').style.display = 'flex'; 
        document.getElementById('tel-pazarlik-btn').style.display = 'none'; 
    } else { 
        oyunSesi('hata'); 
        document.getElementById('tel-diyalog').innerHTML = `<span style="color:#e74c3c">"Ölücüler aramasın kardeşim hadi eyvallah!"</span>`; 
        arabalar = arabalar.filter(x => x.id !== id); 
        piyasayiYenile(); 
    }
}

function hakanAbiAra(id) {
    if(garaj.length >= aracKapasitesi) return; 
    const a = arabalar.find(x => x.id === id); 
    modaliKapat('ilan-detay-modal'); 
    document.getElementById('tel-aranan-kisi').innerText = `👑 Hakan Abi`; 
    document.getElementById('tel-aksiyonlar').style.display = 'none'; 
    document.getElementById('telefon-modal').style.display = 'block'; 
    document.querySelector('.telefon-ekrani').classList.add('caliyor');
    ...
    setTimeout(() => { 
        document.querySelector('.telefon-ekrani').classList.remove('caliyor'); 
        let ind = Math.floor(a.fiyat * 0.6); 
        document.getElementById('tel-diyalog').innerHTML = `"Hallettim aslanım, ${ind.toLocaleString()} TL ver al arabayı."`; 
        document.getElementById('tel-aksiyonlar').style.display = 'flex'; 
        document.getElementById('tel-pazarlik-btn').style.display = 'none'; 
...
function telSatinAl(id) {
    const a = arabalar.find(x => x.id === id); 
    let tm = a.fiyat + noterUcreti + sigortaVeMtvUcreti;
    if(paramiz >= tm) { 
        oyunSesi('kasa'); paramiz -= tm; toplamGider += tm; 
        a.tamirDurumu = 0; garaj.push(a); arabalar = arabalar.filter(x => x.id !== id); 
        ekraniGuncelle(); arabalariEkranaGetir(); oyunuKaydet(); telefonuKapat(); 
        ozelUyari(`Araç garaja çekildi!`, "basari"); 
    } else { 
        oyunSesi('hata'); 
        document.getElementById('tel-diyalog').innerHTML = `<span style="color:#e74c3c">"Araba + Noter parası çıkmıyor sende, vaktimi alma!"</span>`; 
        document.getElementById('tel-aksiyonlar').style.display = 'none'; 
    }
}
            ...
            if(a.tamirDurumu > 0) { 
                l.innerHTML += `
                    <div class="ilan-karti" style="opacity:0.8; border-left:5px solid #e67e22;">
                        ...
                        <div class="ilan-detay">
                            ...
                            <div style="color:#e67e22; font-weight:bold;">🛠️ Sanayide Yatıyor... (Kalan: ${a.tamirDurumu} Gün)</div>
                        </div>
                    </div>`; 
            } else {
                let ts = a.teklifler ? a.teklifler.length : 0; 
                let mods = ''; 
                a.modifiyeler.forEach(m => { mods += `<span class="etiket" style="background:#102a43; margin-right:5px;">${m}</span>`; });
                let hasarM = a.hasarli ? '<span class="etiket etiket-kirmizi">Ağır Hasarlı</span>' : '<span class="etiket etiket-yesil">Sorunsuz</span>';
                ...
                l.innerHTML += `
                    <div class="ilan-karti">
                        ...
                        <div class="ilan-detay">
                            <h3 class="ilan-baslik">${a.marka} ${a.model}</h3>
                            <div class="ilan-ozellikler"><span>🗓️ ${a.yil}</span><span>🛣️ ${a.km.toLocaleString()} KM</span></div>
                            <div style="margin-top:5px;">${hasarM}<br><div style="margin-top:5px;">${mods}</div></div>
                            <div style="margin-top:8px; font-size:13px; color:#627d98;">Maliyet: ${a.fiyat.toLocaleString()} ₺</div>
                            <div style="margin-top:5px; font-size:13px; font-weight:bold; color:#00b894;">🔥 ${ts} Yeni Teklif</div>
                        </div>
                        <div class="ilan-sag-taraf">
                            ${a.hasarli ? `<button class="btn btn-turuncu" onclick="tamirEt(${a.id})">🛠️ Sanayiye Ver</button>` : ''}
                            <button class="btn btn-mor" onclick="modifiyeEkraniAc(${a.id})">✨ Modifiye</button>
                            <button class="btn btn-kirmizi" onclick="araciSat(${a.id})">🤝 Teklifleri Gör</button>
                        </div>
                    </div>`;
            }
        });
...
function tamirEt(id) { 
    ...
    if(paramiz >= a.tamirMasrafi) { 
        ...
        ekraniGuncelle(); garajiEkranaGetir(); oyunuKaydet(); ozelUyari("Araç kaportacıya bırakıldı.", "basari"); 
    } else ozelUyari("Ustanın parasını peşin vermen lazım!", "hata"); 
}

function modifiyeEkraniAc(id) {
    const a = garaj.find(x => x.id === id); 
    const l = document.getElementById('modifiye-listesi'); l.innerHTML = ''; let y = false;
        ...
        if(!a.modifiyeler.includes(p.isim)) { 
            ...
            l.innerHTML += `
                <div style="display:flex; justify-content:space-between; background:#f0f4f8; padding:15px; border-radius:12px; margin-bottom:10px; align-items:center;">
                    <div><span style="font-size:20px;">${p.ikon}</span> <strong>${p.isim}</strong><br><span style="color:#d63031;">${p.maliyet.toLocaleString()} ₺</span></div>
                    <button class="btn btn-mavi" style="width:auto;" onclick="modifiyeUygula(${a.id}, ${p.id})">Uygula</button>
                </div>`; 
        } 
    });
    if(!y) l.innerHTML = `<p style="text-align:center; color:#e67e22; font-weight:bold;">Araç en "gırtlak dolu" halinde!</p>`; 
    document.getElementById('modifiye-modal').style.display = 'block';
}

function modifiyeUygula(aid, pid) { 
    const a = garaj.find(x => x.id === aid); 
    const p = modifiyePaketleri.find(x => x.id === pid); 
    if(paramiz >= p.maliyet) { 
        oyunSesi('tamir'); paramiz -= p.maliyet; toplamGider += p.maliyet; 
        a.fiyat += p.degerArtisi; a.modifiyeler.push(p.isim); 
        ekraniGuncelle(); garajiEkranaGetir(); modaliKapat('modifiye-modal'); oyunuKaydet(); 
        ozelUyari("Modifiye araca başarıyla eklendi!", "basari"); 
    } else ozelUyari("Bunun için kasanızda yeterli para yok.", "hata"); 
}

function araciSat(id) {
    const a = garaj.find(x => x.id === id); 
    const l = document.getElementById('teklif-listesi'); 
    l.innerHTML = `<p style="color:#627d98;">Aracın Sana Maliyeti: <strong>${a.fiyat.toLocaleString()} TL</strong></p>`;

    if(!a.teklifler || a.teklifler.length === 0) { 
        l.innerHTML += `<p style="text-align:center; color:#e74c3c; font-weight:bold;">Henüz teklif yok. Sonraki Gün'e geç.</p>`; 
    } else {
        a.teklifler.sort((x,y) => y.fiyat - x.fiyat).forEach(t => {
            let et = t.tip === "Tayfa" ? `<span class="etiket" style="background:#243b53; color:#f1c40f;">Piyasa Tayfası</span>` : t.tip === "Olucu" ? `<span class="etiket etiket-kirmizi">Ölücü</span>` : t.tip === "Takas" ? `<span class="etiket etiket-yesil">Takas</span>` : '';
            ...
            l.innerHTML += `
                <div id="${t.id}" style="background:#f0f4f8; padding:15px; border-radius:12px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
                    <div><div style="color:#0984e3; font-weight:bold; font-size:16px;">👤 ${t.musteri} ${et}</div>
                    ...
                    <div style="display:flex; flex-direction:column; gap:8px;">
                        ...
                        <button class="btn btn-turuncu" onclick="pazarlikYapp(${a.id}, '${t.id}')">Pazarlık Yap</button>
                    </div>
                </div>`;
        });
    } 
...
function pazarlikYapp(aid, tid) {
    const a = garaj.find(x => x.id === aid); 
    const ix = a.teklifler.findIndex(x => x.id === tid); 
    const t = a.teklifler[ix]; 
    const div = document.getElementById(tid);
    
    if(t.tip === "Olucu") { 
        oyunSesi('hata'); a.teklifler.splice(ix,1); oyunuKaydet(); 
        div.innerHTML = `<div style="color:#d63031; font-weight:bold; text-align:center; width:100%;">"O parayı vereceğime bisiklet alırım" 😡 (Gitti)</div>`; return; 
    }
    
    if(Math.random() > 0.5) { 
        t.fiyat = Math.floor(t.fiyat * (1 + ((Math.floor(Math.random()*8)+5)/100))); 
        oyunSesi('kasa'); oyunuKaydet(); 
        let gr = t.tip === "Takas" ? `<span style="font-size:12px; color:#627d98;">${t.takasArac.marka} + </span><br>${t.fiyat.toLocaleString()} ₺` : `${t.fiyat.toLocaleString()} ₺`;
        div.innerHTML = `
            <div><div style="color:#0984e3; font-weight:bold;">👤 ${t.musteri} (İkna Oldu!)</div>
            <div style="font-size:20px; font-weight:bold; color:#00b894; margin-top:5px;">${gr}</div></div>
            <button class="btn btn-yesil" style="width:auto;" onclick="teklifiKabulEt(${a.id}, '${t.id}')">Kabul Et</button>`; 
    } else { 
        oyunSesi('hata'); a.teklifler.splice(ix,1); oyunuKaydet(); 
        div.innerHTML = `<div style="color:#d63031; font-weight:bold; text-align:center; width:100%;">Müşteri sinirlendi ve gitti!</div>`; 
    }
}

function teklifiKabulEt(aid, tid) {
    const a = garaj.find(x => x.id === aid); 
    const t = a.teklifler.find(x => x.id === tid); 
    let nk = t.fiyat - noterUcreti; 
    modaliKapat('teklif-modal'); oyunSesi('kasa'); 
    paramiz += nk; toplamGelir += nk; toplamGider += noterUcreti; toplamSatilanArac++; 
    garaj = garaj.filter(x => x.id !== aid);
    
    if(t.tip === "Takas") { 
        if(garaj.length >= aracKapasitesi) ozelUyari("Garajda yer yok! Takasla gelen aracı sokakta bıraktın ve çalındı!", "hata"); 
        else { garaj.push(t.takasArac); ozelUyari(`Takas başarılı!\nÜste Alınan Nakit: ${nk.toLocaleString()} ₺\n${t.takasArac.marka} garaja çekildi.`, "basari"); } 
    } else { 
        ozelUyari(`Araç Satıldı!\nKasa Girişi: ${nk.toLocaleString()} TL`, "basari"); 
    }
    ekraniGuncelle(); garajiEkranaGetir(); oyunuKaydet();
}

function dukkanEkraniniGuncelle() {
    const s = seviyeler[dukkanSeviyesi-1]; 
    document.getElementById('dukkan-isim').innerText = `${s.isim} (Seviye ${s.seviye})`; 
    document.getElementById('dukkan-kapasite').innerText = s.kapasite === 999 ? "Sınırsız Araç" : `${s.kapasite} Araç`;
    
    if(dukkanSeviyesi < seviyeler.length) { 
        document.getElementById('yukseltme-alani').style.display = 'block'; 
        document.getElementById('yeni-seviye-isim').innerText = `${seviyeler[dukkanSeviyesi].isim}`; 
        document.getElementById('yeni-seviye-fiyat').innerText = `${seviyeler[dukkanSeviyesi].fiyat.toLocaleString()} ₺`; 
    } else { 
        document.getElementById('yukseltme-alani').innerHTML = `<h3 style="color:#00b894;">🎉 Maksimum Seviyedesin!</h3>`; 
    }
}

function dukkaniYukselt() { 
    if(dukkanSeviyesi >= seviyeler.length) return; 
    const s = seviyeler[dukkanSeviyesi]; 
    if(paramiz >= s.fiyat) { 
        paramiz -= s.fiyat; dukkanSeviyesi++; aracKapasitesi = s.kapasite; 
        ekraniGuncelle(); dukkanEkraniniGuncelle(); oyunuKaydet(); oyunSesi('kasa'); 
        ozelUyari("Dükkan başarıyla büyütüldü!", "basari"); 
    } else ozelUyari("Dükkanı büyütmek için paranız yetersiz!", "hata"); 
}
...
function istatistikleriGuncelle() { 
    ...
    let nk = toplamGelir - toplamGider; 
    document.getElementById('ist-net').innerText = nk.toLocaleString('tr-TR'); 
    document.getElementById('ist-net').style.color = nk < 0 ? '#d63031' : '#00b894'; 
}

// SOSYAL MEDYA 3.0 FONKSİYONLARI 
function sosyalEkraniGuncelle() {
    if (!sosyalMedya.aktif) { 
        ...
        if (sosyalMedya.takipci >= 100000 && !sosyalMedya.sponsorlukAktif) { 
            sBtn.disabled = false; sBtn.innerText = "Anlaşma İmzala"; 
        } else if (sosyalMedya.sponsorlukAktif) { 
            sBtn.disabled = true; sBtn.innerText = "Sponsor Aktif"; 
            document.getElementById('sponsorluk-alani').style.display = 'block'; 
        } else { 
            sBtn.disabled = true; sBtn.innerText = "100k Takipçi Lazım"; 
        }
        
        const kutu = document.getElementById('dm-kutusu'); kutu.innerHTML = '';
        if (dmKutusu.length === 0) {
            kutu.innerHTML = '<p style="text-align:center; color:#829ab1;">Mesaj kutun boş.</p>';
        } else { 
            dmKutusu.forEach((m, i) => { 
                kutu.innerHTML += `
                    <div class="ilan-karti" style="border-left: 5px solid #0984e3;">
                        <div class="ilan-detay">
                            <div style="font-weight: bold; font-size:16px;">📩 ${m.gonderen}</div>
                            <div style="font-size: 13px; color: #627d98;">"${m.metin}"</div>
                            <div style="font-weight: bold; color: #00b894; margin-top: 8px;">Teklif: ${m.teklifFiyat.toLocaleString()} ₺</div>
                        </div>
                        <div class="ilan-sag-taraf">
                            <button class="btn btn-yesil" style="margin-bottom:5px;" onclick="dmTeklifKabul(${i}, ${m.arabaId})">Onayla</button>
                            <button class="btn btn-kirmizi" onclick="dmSil(${i})">Sil</button>
                        </div>
                    </div>`; 
            }); 
        }
...
function sosyalHesapAc() { 
    ...
    if (ka.trim() === "") return ozelUyari("Lütfen kullanıcı adı girin.", "hata"); 
    
    sosyalMedya.aktif = true; sosyalMedya.platform = plat; 
    sosyalMedya.kullaniciAdi = ka.startsWith('@') ? ka : '@'+ka; 
    sosyalMedya.takipci = 15; 
...
function dmTeklifKabul(i, aId) { 
    ...
    if (aIx === -1) { ozelUyari("Müşterinin istediği araç artık garajda yok!", "hata"); dmSil(i); return; } 
    
    let nk = dmKutusu[i].teklifFiyat - noterUcreti; 
    paramiz += nk; toplamGelir += nk; toplamGider += noterUcreti; toplamSatilanArac++; 
    garaj.splice(aIx, 1); dmKutusu.splice(i, 1); 
    
    oyunSesi('kasa'); ekraniGuncelle(); garajiEkranaGetir(); sosyalEkraniGuncelle(); oyunuKaydet(); 
    ozelUyari(`Sosyal medya üzerinden satış tamam! +${nk.toLocaleString()} TL`, "basari"); 
}

function maviTikAl() { 
    if(sosyalMedya.maviTik) return ozelUyari("Zaten Mavi Tik'in var!", "bilgi"); 
    if(paramiz < 25000) return ozelUyari("Hesabında 25.000 TL nakit olmalı.", "hata"); 
    paramiz -= 25000; toplamGider += 25000; sosyalMedya.maviTik = true; 
    oyunSesi('kasa'); sosyalEkraniGuncelle(); ekraniGuncelle(); oyunuKaydet(); 
    ozelUyari("Hesabın doğrulandı! (☑️)", "basari"); 
}

function ozurVideosuCek() { 
    if (paramiz < 30000) return ozelUyari("PR ajansına verecek 30.000 TL paran yok!", "hata"); 
    paramiz -= 30000; toplamGider += 30000; sosyalMedya.lincBitisGunu = gun; 
    let kayip = Math.floor(sosyalMedya.takipci * 0.1); sosyalMedya.takipci -= kayip; 
    oyunSesi('kasa'); sosyalEkraniGuncelle(); ekraniGuncelle(); oyunuKaydet(); 
    ozelUyari(`Ağlayarak özür videosu çektin. Linç bitti ama ${kayip} takipçi kaybettin.`, "basari"); 
}

function sponsorlukAnlasmasi() { 
    sosyalMedya.sponsorlukAktif = true; oyunSesi('kasa'); sosyalEkraniGuncelle(); oyunuKaydet(); 
    ozelUyari("Markalarla sponsorluk anlaşması imzaladın! Artık her gün pasif gelir yatacak.", "basari"); 
}

function icerikSecimEkraniAc() {
    if(garaj.length === 0) return ozelUyari("Garajda araba yok, neyin videosunu çekeceksin!", "hata");
    ...
    garaj.forEach(a => { 
        l.innerHTML += `
            <div class="ilan-karti" style="cursor:pointer;" onclick="videoCek(${a.id})">
                ...
                <button class="btn btn-yesil" style="width:auto; margin-left:auto;">Bunu Çek</button>
            </div>`; 
    });
...
function videoCek(aId) {
    ...
    if(paramiz < m) return ozelUyari("Bu videoyu çekmek için kasanızda yeterli para yok!", "hata");
    
    paramiz -= m; toplamGider += m; modaliKapat('icerik-secim-modal'); oyunSesi('kasa');
    
    if(tip === "normal") { 
        t = Math.floor(Math.random()*300)+100; 
        ozelUyari(`Harika bir şov videosu atıldı. +${t} takipçi kazandın!`, "basari"); 
    } else if(tip === "teslimat") { 
        t = Math.floor(Math.random()*500)+200; 
        ozelUyari(`Müşteriye konfetili teslimat şovu çektin. Güvenin arttı, +${t} takipçi geldi!`, "basari"); 
    } else if(tip === "ifsa") {
        if(Math.random() < 0.20) { 
            let ceza = Math.floor(paramiz * 0.1); paramiz -= ceza; toplamGider += ceza; 
            sosyalMedya.lincBitisGunu = gun + 3; oyunSesi('hata'); 
            ozelUyari(`🚨 İFŞA TERS TEPTİ! İfşa ettiğin galericiler dükkanı bastı. Camı çerçeveyi indirdiler. ${ceza.toLocaleString()} TL hasar ödedin ve linç yiyorsun!`, "hata"); 
            return; 
        }
        t = Math.floor(Math.random()*5000)+2000; 
        ozelUyari(`🔥 İfşa videon fena viral oldu! Herkes seni konuşuyor. Bir anda +${t.toLocaleString()} takipçi kazandın!`, "basari");
    }
...
function canliYayinSecimAc() {
    if(garaj.length === 0) return ozelUyari("Garajda satılacak araç yok!", "hata");
    if(gun < sosyalMedya.lincBitisGunu) return ozelUyari("Linç yerken yayın açamazsın, herkes küfreder!", "hata");
    ...
    garaj.forEach(a => { 
        l.innerHTML += `
            <div class="ilan-karti" style="cursor:pointer;" onclick="canliYayinaBasla(${a.id})">
                ...
                <div><h3 style="margin:0; font-size:14px;">${a.marka} ${a.model}</h3><div style="font-size:12px; color:#00b894;">Maliyetin: ${a.fiyat.toLocaleString()} ₺</div></div>
                <button class="btn btn-kirmizi" style="width:auto; margin-left:auto;">Yayına Çıkar</button>
            </div>`; 
    });
...
function canliYayinaBasla(aId) {
    ...
    const a = garaj.find(x => x.id === aId); yayinArabaId = a.id;
    yayinTuru = 0; 
    anlikFiyat = Math.floor(a.fiyat * 0.75); // Araç maliyetin %25 altından açık artırmaya girer
    yayinIzleyici = Math.floor(sosyalMedya.takipci * 0.05) + Math.floor(Math.random()*500);
    
    document.getElementById('yayin-foto').src = a.gorsel;
    document.getElementById('yayin-araba-isim').innerText = `${a.marka} ${a.model}`;
    document.getElementById('yayin-guncel-teklif').innerText = `${anlikFiyat.toLocaleString()} ₺`;
    document.getElementById('yayin-kisi-sayisi').innerText = yayinIzleyici.toLocaleString();
    document.getElementById('yayin-yorum-alani').innerHTML = '';
    
    document.getElementById('canli-yayin-modal').style.display = 'block';
    
    yayinInterval = setInterval(() => {
        yayinTuru++;
        if (yayinTuru > 15) { // Çok beklenirse seyirci sıkılıp kaçar
            yayinIzleyici = Math.floor(yayinIzleyici * 0.8);
            if (Math.random() > 0.5) anlikFiyat = Math.floor(anlikFiyat * 0.98); 
        } else {
            yayinIzleyici += Math.floor(Math.random() * 50);
            if (Math.random() > 0.3) {
                let artis = Math.floor(anlikFiyat * (Math.random() * 0.04 + 0.01)); 
                anlikFiyat += artis;
                oyunSesi('kasa');
                ...
                let txt = `<div class="yayin-yorum" style="color:#fff;"><strong style="color:#00d2d3;">${kisi}</strong>: Benden ${anlikFiyat.toLocaleString()} ₺ çalışır!</div>`;
                ...
                document.querySelector('.canli-yayin-ekrani').appendChild(kalp);
        ...
        if(yayinIzleyici < 10) yayiniKapat("Seyirci kalmadı, kimse teklif vermiyor. Yayın bitti.");
    }, 1500);
}

function yayindaSat() {
    clearInterval(yayinInterval); 
    const aIx = garaj.findIndex(x => x.id === yayinArabaId);
    let nk = anlikFiyat - noterUcreti; 
    paramiz += nk; toplamGelir += nk; toplamGider += noterUcreti; toplamSatilanArac++;
    garaj.splice(aIx, 1); 
    oyunSesi('kasa'); ekraniGuncelle(); garajiEkranaGetir(); oyunuKaydet();
    modaliKapat('canli-yayin-modal'); 
    ozelUyari(`🎉 Canlı yayında araç açık artırmayla SATILDI!\nNet Kasa Girişi: ${nk.toLocaleString()} TL`, "basari");
}

function yayiniKapat(msj = null) { 
    clearInterval(yayinInterval); 
    modaliKapat('canli-yayin-modal'); 
    if(msj) ozelUyari(msj, "bilgi"); 
}

// OYUN BAŞLATMA ALTYAPISI
function oyunuBaslat() { 
    if (!oyunuYukle()) { 
        // Eski kayıt yoksa veya bozuksa başlangıç ekranını göster
        document.getElementById('baslangic-modal').style.display = 'block'; 
        piyasayiYenile(); // Arka planda piyasaya araçları koy
    } else { 
        document.getElementById('header-logo').innerHTML = `${galeriAdi}<span>Motors</span>`; 
        ekraniGuncelle(); 
        menuDegistir('pazar'); // Ana sayfayı yükle
    } 
}

function galeriAdiniKaydet() { 
    let girilenAd = document.getElementById('galeri-adi-input').value; 
    if (girilenAd.trim() === "") { ozelUyari("Galerin için bir isim girmelisin!", "hata"); return; } 
    galeriAdi = girilenAd; 
