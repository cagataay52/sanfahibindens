...
function teklifiKabulEt(aid, tid) {
    ...
    if(t.tip==="Takas") { if(garaj.length>=aracKapasitesi) ozelUyari("Garaj dolu, takas arabası sokakta çalındı!", "hata"); else { garaj.push(t.takasArac); ozelUyari(`Takas başarılı! +${nk.toLocaleString()} ₺ ve ${t.takasArac.marka} alındı.`, "basari"); } } else ozelUyari(`Araç Satıldı! +${nk.toLocaleString()} ₺`, "basari");
...
// -------------------------------------------------------------------------
// SOSYAL MEDYA 3.0: CANLI YAYIN, İÇERİK KONSEPTLERİ VE SPONSORLUK
// -------------------------------------------------------------------------
...
function ozurVideosuCek() { if (paramiz < 30000) return ozelUyari("PR ajansına verecek 30.000 TL yok!", "hata"); paramiz -= 30000; toplamGider += 30000; sosyalMedya.lincBitisGunu = gun; let kayip = Math.floor(sosyalMedya.takipci * 0.1); sosyalMedya.takipci -= kayip; oyunSesi('kasa'); sosyalEkraniGuncelle(); ekraniGuncelle(); oyunuKaydet(); ozelUyari(`Ağlayarak özür diledin. Linç bitti ama ${kayip} takipçi kaybettin.`, "basari"); }
function sponsorlukAnlasmasi() { sosyalMedya.sponsorlukAktif = true; oyunSesi('kasa'); sosyalEkraniGuncelle(); oyunuKaydet(); ozelUyari("Markalarla anlaştın! Artık her gün 15.000 TL hesabına yatacak.", "basari"); }
...
function icerikSecimEkraniAc() {
    ...
    garaj.forEach(a => { l.innerHTML+=`<div class="ilan-karti" style="cursor:pointer;" onclick="videoCek(${a.id})"><img src="${a.gorsel}" style="width:80px; height:60px; object-fit:cover; border-radius:8px; margin-right:15px;"><div><h3 style="margin:0; font-size:14px;">${a.marka} ${a.model}</h3></div></div>`; });
...
function videoCek(aId) {
    ...
    if(tip==="normal") { t = Math.floor(Math.random()*300)+100; ozelUyari(`Şov videosu atıldı. +${t} takipçi!`, "basari"); }
    else if(tip==="teslimat") { t = Math.floor(Math.random()*500)+200; ozelUyari(`Müşteriye konfetili teslimat çektin. Güven arttı, +${t} takipçi!`, "basari"); }
    else if(tip==="ifsa") {
        if(Math.random()<0.20) { let ceza=Math.floor(paramiz*0.1); paramiz-=ceza; toplamGider+=ceza; sosyalMedya.lincBitisGunu=gun+3; oyunSesi('hata'); ozelUyari(`🚨 YANLIŞ İFŞA! İfşa ettiğin galericiler dükkanı bastı. Camı çerçeveyi indirdiler. ${ceza.toLocaleString()} TL hasar ve linç yedin!`, "hata"); return; }
        t = Math.floor(Math.random()*5000)+2000; ozelUyari(`🔥 İfşa videon viral oldu! Herkes seni konuşuyor. +${t.toLocaleString()} takipçi!`, "basari");
    }
    sosyalMedya.takipci += t; sosyalEkraniGuncelle(); ekraniGuncelle(); oyunuKaydet();
}

// CANLI YAYIN MEZAT SİSTEMİ
...
function canliYayinaBasla(aId) {
    ...
    yayinTuru = 0; anlikFiyat = Math.floor(a.fiyat * 0.75); // %25 indirimli başlar
    ...
    yayinInterval = setInterval(() => {
        yayinTuru++;
        if (yayinTuru > 15) { // Seyirci sıkılır
            yayinIzleyici = Math.floor(yayinIzleyici * 0.8);
            if (Math.random() > 0.5) anlikFiyat = Math.floor(anlikFiyat * 0.98); // Fiyat düşer
        } else {
            yayinIzleyici += Math.floor(Math.random() * 50);
            if (Math.random() > 0.3) {
                let artis = Math.floor(anlikFiyat * (Math.random() * 0.04 + 0.01)); // %1 ile %4 arası artış
                anlikFiyat += artis;
                oyunSesi('kasa');
                
                // Animasyonlu Yorum Ekle
                ...
                let txt = `<div class="yayin-yorum" style="color:#fff;"><strong style="color:#00d2d3;">${kisi}</strong>: ${anlikFiyat.toLocaleString()} ₺ veriyorum!</div>`;
...
function yayindaSat() {
    clearInterval(yayinInterval); const aIx = garaj.findIndex(x=>x.id===yayinArabaId);
    let nk = anlikFiyat - noterUcreti; paramiz += nk; toplamGelir += nk; toplamGider += noterUcreti; toplamSatilanArac++;
    garaj.splice(aIx, 1); oyunSesi('kasa'); ekraniGuncelle(); garajiEkranaGetir(); oyunuKaydet();
    modaliKapat('canli-yayin-modal'); ozelUyari(`Canlı yayında araç SATILDI!\nKasa Girişi: ${nk.toLocaleString()} TL`, "basari");
}
