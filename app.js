...
function uyariyiKapat() { let m=document.getElementById('uyari-modal'); if(m) m.style.display = 'none'; }
function modaliKapat(modalId) { let m=document.getElementById(modalId); if(m) m.style.display = "none"; }
function ayarlarModalAc() { let m=document.getElementById('ayarlar-modal'); if(m) m.style.display = "block"; }
...
function ekonomiOlayiTetikle() { 
    ...
    else if (sans > 0.40 && sans < 0.45 && piyasaDurumu !== "Normal") { piyasaDurumu = "Normal"; piyasaCarpani = 1.0; ozelUyari("⚖️ Piyasa normale döndü.", "bilgi"); } 
...
// BÜTÜN EKRANLARI HATA OLMADAN GÜNCELLEYEN MERKEZİ SİSTEM
function aktifEkraniYenile() {
    try {
        ...
        let bnk = document.getElementById('banka-ekrani'); if(bnk && bnk.style.display === 'block') { senetleriEkranaBas(); let kur = document.getElementById('euro-kur-ekran'); if(kur) kur.innerText = euroKuru.toFixed(2); let brc = document.getElementById('borc-miktari'); if(brc) brc.innerText = bankaBorcu.toLocaleString('tr-TR'); }
...
function ekraniGuncelle() {
    ...
    if(kps) {
        ...
        if (toplamAracSayisi >= aracKapasitesi && aracKapasitesi !== 999) kps.style.color = '#e74c3c'; else kps.style.color = '#0984e3';
    }
...
// KIRILMAZ GÜN ATLATMA SİSTEMİ
function sonrakiGun() {
    try {
        gun++; let g = document.getElementById('gun'); if(g) g.innerText = gun;
        
        if (personeller.smUzman && sosyalMedya.aktif) { sosyalMedya.takipci += Math.floor(Math.random() * 150) + 50; }
        ...
        garaj.forEach(araba => {
            if (araba.tamirDurumu > 0) {
                ...
                else if (Math.random() < 0.20 && !personeller.usta) { let ekstra = Math.floor(araba.tamirMasrafi * 0.5); if (paramiz >= ekstra) { paramiz -= ekstra; toplamGider += ekstra; araba.tamirDurumu += 2; oyunSesi('hata'); ozelUyari(`📞 Hamza Usta: "Sana ${ekstra.toLocaleString('tr-TR')} ₺ daha kilitledim."`, "hata"); } }
            } else {
        ...
        if (sosyalMedya.aktif && garaj.length > 0) {
            if (sosyalMedya.lincKalanGun > 0) { sosyalMedya.lincKalanGun--; } 
                ...
                if (Math.random() < dmIhtimali) {
                    ...
                    dmKutusu.push({ gonderen: "@" + musteriIsimleri[Math.floor(Math.random() * musteriIsimleri.length)].toLowerCase() + Math.floor(Math.random()*99), metin: `Reis araba duruyor mu?`, teklifFiyat: teklif, arabaId: sansliAraba.id });
                }
            }
        }
    } catch(e) { 
        console.error("Gün atlatma hatası:", e); 
    } finally {
        // Hata olsa bile oyun yenilenir ve kaydedilir!
        piyasayiYenile(); ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); 
    }
}
...
function menuDegistir(menu) {
    ...
    aktifEkraniYenile();
    if(window.innerWidth <= 768) { let sm = document.querySelector('.sol-menu'); if(sm) sm.classList.remove('acik'); let ov = document.getElementById('mobil-menu-overlay'); if(ov) ov.classList.remove('acik'); }
}
...
function rentACarEkraniGuncelle() { const liste = document.getElementById('rentacar-listesi'); const bilgi = document.getElementById('rentacar-bilgi'); if(!liste) return; liste.innerHTML = ''; if (rentACarFilosu.length === 0) { if(bilgi) bilgi.style.display = 'block'; } else { if(bilgi) bilgi.style.display = 'none'; let toplam = 0; rentACarFilosu.forEach(a => { toplam += a.gunlukKiraBedeli || 0; liste.innerHTML += `<div class="ilan-karti" style="border-left: 5px solid #d35400;"><div class="araba-foto"><img src="${a.gorsel}" style="width:100%; height:100%; object-fit:cover; border-radius:8px;"></div><div class="ilan-detay"><h3 class="ilan-baslik">${a.marka} ${a.model}</h3><div style="margin-top:8px; color:#27ae60; font-size:16px; font-weight: bold;">Günlük Getiri: +${(a.gunlukKiraBedeli||0).toLocaleString('tr-TR')} ₺</div></div><div class="ilan-sag-taraf"><button class="btn btn-turuncu" onclick="kiradanCek(${a.id})">Kiradan Çek</button></div></div>`; }); liste.innerHTML = `<h3 style="color:#27ae60; text-align:center;">Toplam Günlük Pasif Gelir: ${toplam.toLocaleString('tr-TR')} ₺</h3>` + liste.innerHTML; } }
function rentACarGelirVeRiskYonetimi() { if (rentACarFilosu.length === 0) return; let kazanilan = 0; for (let i = rentACarFilosu.length - 1; i >= 0; i--) { let a = rentACarFilosu[i]; let risk = Math.random(); if (risk < 0.02) { oyunSesi('hata'); ozelUyari(`🚨 ŞOK! Kiradaki ${a.marka} aracın çalındı!`, "hata"); rentACarFilosu.splice(i, 1); } else if (risk < 0.07) { a.hasarli = true; a.tamirMasrafi = Math.floor(a.fiyat * 0.20); a.fiyat = Math.floor(a.fiyat * 0.70); garaj.push(a); rentACarFilosu.splice(i, 1); oyunSesi('hata'); ozelUyari(`🚨 KAZA! Müşteri ${a.marka} ile kaza yapmış!`, "hata"); } else if (risk < 0.15) { let ceza = Math.floor((a.gunlukKiraBedeli||0) * 2); paramiz -= ceza; toplamGider += ceza; kazanilan += (a.gunlukKiraBedeli||0); ozelUyari(`⚠️ Kiradaki ${a.marka} hor kullanılmış. ${ceza} ₺ masraf.`, "bilgi"); } else { kazanilan += (a.gunlukKiraBedeli||0); a.km += Math.floor(Math.random() * 200) + 50; } } if (kazanilan > 0) { paramiz += kazanilan; toplamGelir += kazanilan; oyunSesi('kasa'); } }
...
function hakanAbiTahsilat(i) { let s = senetler[i]; let kalan = s.toplamBorc - s.odenen; let komisyon = Math.floor(kalan * 0.20); paramiz += (kalan - komisyon); toplamGelir += (kalan - komisyon); s.odenen += kalan; s.kalanGun = 0; s.durum = 'Bitti (Tahsil Edildi)'; oyunSesi('kasa'); ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Hakan Abi tahsilatı yaptı!`, "basari"); }

// PERSONEL
function personelEkraniGuncelle() { 
    const bUsta = document.getElementById('btn-personel-usta'); const bSm = document.getElementById('btn-personel-sm'); const bSatis = document.getElementById('btn-personel-satis');
    if(bUsta) { if(personeller.usta) bUsta.innerHTML = `<button class="btn btn-kirmizi" onclick="personelKov('usta')">Kov</button> <br>✅ Çalışıyor`; else bUsta.innerHTML = `<button class="btn btn-yesil" onclick="personelIseAl('usta')">İşe Al</button>`; }
    if(bSm) { if(personeller.smUzman) bSm.innerHTML = `<button class="btn btn-kirmizi" onclick="personelKov('smUzman')">Kov</button> <br>✅ Çalışıyor`; else bSm.innerHTML = `<button class="btn btn-yesil" onclick="personelIseAl('smUzman')">İşe Al</button>`; }
    if(bSatis) { if(personeller.satisTemsilci) bSatis.innerHTML = `<button class="btn btn-kirmizi" onclick="personelKov('satisTemsilci')">Kov</button> <br>✅ Çalışıyor`; else bSatis.innerHTML = `<button class="btn btn-yesil" onclick="personelIseAl('satisTemsilci')">İşe Al</button>`; }
}
function personelIseAl(rol) { let m = { usta: 25000, smUzman: 15000, satisTemsilci: 30000 }; if(paramiz < m[rol]) { ozelUyari("Para yok!", "hata"); return; } paramiz -= m[rol]; toplamGider += m[rol]; personeller[rol] = true; oyunSesi('kasa'); ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`İşe alındı!`, "basari"); }
...
function dukkaniYukselt() { if (dukkanSeviyesi >= seviyeler.length) return; const son = seviyeler[dukkanSeviyesi]; if (paramiz >= son.fiyat) { oyunSesi('tamir'); paramiz -= son.fiyat; toplamGider += son.fiyat; dukkanSeviyesi++; aracKapasitesi = son.kapasite; ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Dükkan büyüdü!`, "basari"); } else { oyunSesi('hata'); ozelUyari("Para yok!", "hata"); } }
function krediCek(m) { paramiz+=m; bankaBorcu+=m; oyunSesi('kasa'); ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Kredi çekildi.`, 'basari'); } 
function borcOde(m) { if(bankaBorcu===0)return; if(paramiz>=m){ let od=m>bankaBorcu?bankaBorcu:m; paramiz-=od; bankaBorcu-=od; oyunSesi('kasa'); ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); }else{ ozelUyari("Para yok!", "hata"); } } 
function borcuKapat() { if(bankaBorcu===0)return; if(paramiz>=bankaBorcu){ paramiz-=bankaBorcu; bankaBorcu=0; oyunSesi('kasa'); ekraniGuncelle(); aktifEkraniYenile(); oyunuKaydet(); }else{ ozelUyari("Para yok!", "hata"); } }
...
function ilanDetayEkraniAc(id) { const a = arabalar.find(x => x.id === id); document.getElementById('detay-foto').src = a.gorsel; document.getElementById('detay-baslik').innerText = `Satılık ${a.marka} ${a.model}`; document.getElementById('detay-fiyat').innerText = `${a.fiyat.toLocaleString('tr-TR')} TL`; document.getElementById('detay-marka').innerText = a.marka; document.getElementById('detay-model').innerText = a.model; document.getElementById('detay-yil').innerText = a.yil; document.getElementById('detay-km').innerText = a.km.toLocaleString('tr-TR'); document.getElementById('detay-aciklama').innerText = a.ilanAciklamasi; document.getElementById('detay-telefon').innerText = a.telefon; ['kaput', 'tavan', 'bagaj', 'solOnCamurluk', 'solOnKapi', 'solArkaKapi', 'solArkaCamurluk', 'sagOnCamurluk', 'sagOnKapi', 'sagArkaKapi', 'sagArkaCamurluk'].forEach(p => { const div = document.getElementById(`eks-${p}`); if(div){ div.className='eks-parca'; div.classList.add(a.ekspertiz[p]); } }); document.getElementById('detay-tramer-btn').onclick = function() { if(paramiz<150){ozelUyari("Para yok!","hata");return;} paramiz-=150; oyunSesi('kasa'); document.getElementById('tramer-mesaj-icerik').innerHTML=a.tramer; document.getElementById('tramer-modal').style.display='block'; }; document.getElementById('detay-satici-ara-btn').onclick = function() { saticiAra(a.id); }; const hk = document.getElementById('detay-hakan-abi-btn'); let kg = 15 - (gun - hakanAbiSonKullanim); if (kg <= 0) { hk.innerText = "👑 Hakan Abi (%40 İndirim)"; hk.style.opacity = "1"; hk.onclick = function() { hakanAbiAra(a.id); }; } else { hk.innerText = `👑 Meşgul (${kg} Gün)`; hk.style.opacity = "0.5"; hk.onclick = function() { ozelUyari(`Meşgul.`, "bilgi"); }; } document.getElementById('ilan-detay-modal').style.display = 'block'; }
let aktifAramaArabaId = null; function telefonuKapat() { modaliKapat('telefon-modal'); document.querySelector('.telefon-ekrani').classList.remove('caliyor'); }
function saticiAra(id) { if ((garaj.length + rentACarFilosu.length) >= aracKapasitesi) { ozelUyari("Kapasite dolu!", "hata"); return; } const a = arabalar.find(x => x.id === id); modaliKapat('ilan-detay-modal'); document.getElementById('tel-aranan-kisi').innerText = `Satıcı Aranıyor`; document.getElementById('tel-diyalog').innerText = "Dıt... Dıt..."; document.getElementById('tel-aksiyonlar').style.display = 'none'; document.querySelector('.telefon-ekrani').classList.add('caliyor'); document.getElementById('telefon-modal').style.display = 'block'; setTimeout(() => { document.querySelector('.telefon-ekrani').classList.remove('caliyor'); document.getElementById('tel-diyalog').innerText = `"Alo buyur kardeşim. Fiyat ${a.fiyat.toLocaleString('tr-TR')} TL."`; document.getElementById('tel-aksiyonlar').style.display = 'flex'; document.getElementById('tel-satin-al-btn').onclick = function() { telSatinAl(a.id); }; document.getElementById('tel-pazarlik-btn').onclick = function() { document.getElementById('tel-aksiyonlar').style.display = 'none'; if (Math.random() > 0.40) { a.fiyat = Math.floor(a.fiyat * (1 - ((Math.floor(Math.random()*8)+3)/100))); oyunSesi('kasa'); document.getElementById('tel-diyalog').innerHTML = `"Hadi ${a.fiyat.toLocaleString('tr-TR')} TL olsun."`; document.getElementById('tel-aksiyonlar').style.display = 'flex'; document.getElementById('tel-pazarlik-btn').style.display = 'none'; } else { oyunSesi('hata'); document.getElementById('tel-diyalog').innerHTML = `"Ölücülerle işim olmaz!"`; arabalar = arabalar.filter(x => x.id !== id); aktifEkraniYenile(); } }; }, 2000); }
...
function telSatinAl(id) { const a = arabalar.find(x => x.id === id); let t = a.fiyat + noterUcreti + sigortaVeMtvUcreti; if (paramiz >= t) { paramiz -= t; a.tamirDurumu = 0; garaj.push(a); arabalar = arabalar.filter(x => x.id !== id); oyunSesi('kasa'); telefonuKapat(); aktifEkraniYenile(); oyunuKaydet(); ozelUyari(`Araç alındı!`, "basari"); } else { oyunSesi('hata'); document.getElementById('tel-diyalog').innerHTML = `"Para çıkışmıyor kardeşim!"`; document.getElementById('tel-aksiyonlar').style.display = 'none'; } }
function garajiEkranaGetir() { const lst = document.getElementById('garaj-listesi'); const blg = document.getElementById('garaj-bilgi'); if(!lst) return; lst.innerHTML = ''; if (garaj.length === 0) { if(blg) blg.style.display = 'block'; } else { if(blg) blg.style.display = 'none'; garaj.forEach(a => { if (a.tamirDurumu > 0) { lst.innerHTML += `<div class="ilan-karti" style="opacity:0.8; border-left:5px solid #e67e22;"><div class="araba-foto"><img src="${a.gorsel}" style="width:100%; height:100%; object-fit:cover; border-radius:8px;"></div><div class="ilan-detay"><h3 class="ilan-baslik">${a.marka} ${a.model} (Sanayide)</h3><div style="color:#e67e22; font-weight: bold;">Kalan: ${a.tamirDurumu} Gün</div></div></div>`; } else { let tek = a.teklifler ? a.teklifler.length : 0; let m = ''; a.modifiyeler.forEach(md => m+=`<span class="etiket" style="background:#2d3436; margin-right:5px;">${md}</span>`); lst.innerHTML += `<div class="ilan-karti"><div class="araba-foto"><img src="${a.gorsel}" style="width:100%; height:100%; object-fit:cover; border-radius:8px;"></div><div class="ilan-detay"><h3 class="ilan-baslik">${a.marka} ${a.model}</h3><div class="ilan-durum">${a.hasarli?'<span class="etiket etiket-kirmizi">Ağır Hasarlı</span>':'<span class="etiket etiket-yesil">Sorunsuz</span>'} ${m}</div><div style="font-size:13px;">${tek>0?`🔥 ${tek} Yeni Teklif!`:`Teklif Bekleniyor`}</div></div><div class="ilan-sag-taraf">${a.hasarli?`<button class="btn btn-turuncu" onclick="tamirEt(${a.id})">🛠️ Sanayiye Ver</button>`:''}<button class="btn" style="background:#2c3e50; color:#f1c40f; margin-bottom:5px;" onclick="kilometreDusur(${a.id})">🤫 KM Düşür</button><button class="btn" style="background:#d35400; color:white; margin-bottom:5px;" onclick="kirayaVer(${a.id})">🔑 Kiraya Ver</button><button class="btn btn-mor" onclick="modifiyeEkraniAc(${a.id})">✨ Modifiye</button><button class="btn btn-kirmizi" onclick="araciSat(${a.id})">🤝 Sat</button></div></div>`; } }); } }

function araciSat(id) { const a = garaj.find(x => x.id === id); const lst = document.getElementById('teklif-listesi'); lst.innerHTML = `<p>Maliyet: <b>${a.fiyat.toLocaleString('tr-TR')} TL</b></p>`; if (!a.teklifler || a.teklifler.length === 0) { lst.innerHTML += `<p>Henüz teklif yok.</p>`; } else { a.teklifler.sort((a,b) => b.fiyat - a.fiyat).forEach(t => { let sF = Math.floor(t.fiyat * 1.30); lst.innerHTML += `<div class="teklif-karti" id="${t.id}"><div><b>👤 ${t.musteri}</b><br><span style="font-size:20px; color:#00b894; font-weight:bold;">${t.fiyat.toLocaleString('tr-TR')} ₺</span></div><div style="display:flex; flex-direction:column; gap:5px;"><button class="btn btn-yesil" onclick="teklifiKabulEt(${id}, '${t.id}')">Nakit Sat</button><button class="btn" style="background:#8e44ad; color:white;" onclick="senetleSat(${id}, '${t.id}', ${sF})">📝 Senetle</button><button class="btn btn-turuncu" onclick="pazarlikYapp(${id}, '${t.id}')">Pazarlık</button></div></div>`; }); } document.getElementById('teklif-modal').style.display = "block"; }
function pazarlikYapp(id, tId) { const a = garaj.find(x => x.id === id); const tI = a.teklifler.findIndex(x => x.id === tId); const t = a.teklifler[tI]; const k = document.getElementById(tId); if (t.tip === "Olucu") { oyunSesi('hata'); a.teklifler.splice(tI, 1); k.innerHTML = `Müşteri küfredip gitti!`; return; } if (Math.random() > 0.5) { t.fiyat = Math.floor(t.fiyat * (1 + ((Math.floor(Math.random()*8)+5)/100))); oyunSesi('kasa'); let sF = Math.floor(t.fiyat * 1.30); k.innerHTML = `<div><b>👤 ${t.musteri} (İkna Oldu)</b><br><span style="font-size:20px; color:#00b894; font-weight:bold;">${t.fiyat.toLocaleString('tr-TR')} ₺</span></div><div style="display:flex; gap:5px;"><button class="btn btn-yesil" onclick="teklifiKabulEt(${id}, '${t.id}')">Nakit Sat</button><button class="btn" style="background:#8e44ad; color:white;" onclick="senetleSat(${id}, '${t.id}', ${sF})">📝 Senetle</button></div>`; } else { oyunSesi('hata'); a.teklifler.splice(tI, 1); k.innerHTML = `Müşteri sinirlenip gitti!`; } oyunuKaydet(); }
function teklifiKabulEt(id, tId) { const a = garaj.find(x => x.id === id); const t = a.teklifler.find(x => x.id === tId); let nk = t.fiyat - noterUcreti; modaliKapat('teklif-modal'); oyunSesi('kasa'); paramiz += nk; toplamGelir += nk; garaj = garaj.filter(x => x.id !== id); if (t.tip === "Takas") { if ((garaj.length + rentACarFilosu.length) >= aracKapasitesi) { ozelUyari(`Araç satıldı ama TAKAS edilen araca yer yok!`, "hata"); } else { garaj.push(t.takasArac); ozelUyari(`Takas Başarılı!`, "basari"); } } else { ozelUyari(`Araç Nakit Satıldı!`, "basari"); } if (gizliKusurluAraclar.includes(id)) { if (Math.random() < 0.40) { let taz = Math.floor(nk * 1.5); paramiz -= taz; haritaPuani -= 1.5; if (haritaPuani < 1.0) haritaPuani = 1.0; oyunSesi('hata'); ozelUyari(`🚨 Müşteri KM düşürüldüğünü anladı! ${taz} ₺ ceza ödedin!`, "hata"); } else { haritaPuani += 0.1; if(haritaPuani>5.0) haritaPuani=5.0; } gizliKusurluAraclar = gizliKusurluAraclar.filter(x => x !== id); } else { haritaPuani += 0.2; if(haritaPuani>5.0) haritaPuani=5.0; } aktifEkraniYenile(); oyunuKaydet(); }
function senetleSat(id, tId, sF) { const a = garaj.find(x => x.id === id); const t = a.teklifler.find(x => x.id === tId); let pes = Math.floor(sF * 0.20); modaliKapat('teklif-modal'); oyunSesi('kasa'); paramiz += pes; toplamGelir += pes; garaj = garaj.filter(x => x.id !== id); senetler.push({ id: 'snt-'+Math.floor(Math.random()*10000), musteri: t.musteri, arabaMarka: a.marka, toplamBorc: sF, odenen: pes, taksit: Math.floor((sF-pes)/10), kalanGun: 10, durum: 'Düzenli Ödüyor' }); ozelUyari(`Araç Senetle Satıldı!`, "basari"); if (gizliKusurluAraclar.includes(id)) { if (Math.random() < 0.40) { haritaPuani -= 1.5; if (haritaPuani < 1.0) haritaPuani = 1.0; } else { haritaPuani += 0.1; } gizliKusurluAraclar = gizliKusurluAraclar.filter(x => x !== id); } else { haritaPuani += 0.2; } if(haritaPuani>5.0) haritaPuani=5.0; aktifEkraniYenile(); oyunuKaydet(); }
...
// CANLI YAYIN 
function canliYayinIcinArabaSec() { if (garaj.length===0||sosyalMedya.takipci<500)return ozelUyari("Uygun değil.","hata"); const l=document.getElementById('yayin-araba-listesi'); if(!l)return; l.innerHTML=''; garaj.forEach(a=>{ l.innerHTML+=`<div class="teklif-karti"><div><b>${a.marka}</b></div><button class="btn btn-kirmizi" onclick="canliYayiniBaslat(${a.id})">Yayınla</button></div>`; }); document.getElementById('yayin-secim-modal').style.display="block"; }
function canliYayiniBaslat(id) { modaliKapat('yayin-secim-modal'); yayindakiAraba=garaj.find(x=>x.id===id); anlikIzleyici=Math.floor(sosyalMedya.takipci*0.1); anlikEnYuksekTeklif=Math.floor(yayindakiAraba.fiyat*0.6); yayinTuruHype=1.0; document.getElementById('yayin-araba-baslik').innerText=yayindakiAraba.marka; document.getElementById('yayin-en-yuksek-teklif').innerText=anlikEnYuksekTeklif; document.getElementById('canli-yayin-modal').style.display="block"; if(yayinInterval)clearInterval(yayinInterval); yayinInterval=setInterval(yayinDongusu, 1500); }
function yayinDongusu() { anlikIzleyici+=Math.floor((Math.random()*20-10)*yayinTuruHype); if(anlikIzleyici<5)anlikIzleyici=5; document.getElementById('yayin-izleyici').innerText=anlikIzleyici; if(yayinTuruHype>1.0)yayinTuruHype-=0.1; let ct=document.getElementById('yayin-chat'); if(Math.random()<(0.35*yayinTuruHype)) { anlikEnYuksekTeklif+=20000; yayindakiTeklifci="Anonim"; document.getElementById('yayin-en-yuksek-teklif').innerText=anlikEnYuksekTeklif.toLocaleString('tr-TR'); ct.innerHTML+=`<div style="color:#00b894;">💰 Benden ${anlikEnYuksekTeklif} ₺!</div>`; oyunSesi('kasa'); } else { ct.innerHTML+=`<div>Yorum...</div>`; } ct.scrollTop=ct.scrollHeight; }
function yayinGazaGetir() { yayinTuruHype=2.5; document.getElementById('yayin-chat').innerHTML+=`<div style="color:#ff7675;">📣 HAYDİ BEYLER!</div>`; }
...
function oyunuBaslat() {
    ...
    else { document.getElementById('header-logo').innerHTML = `${galeriAdi}<span>Motors</span>`; ekraniGuncelle(); if (arabalar.length === 0) piyasayiYenile(); menuDegistir('pazar'); }
}
